    ...
    <div class="flex-1 flex flex-col min-h-screen lg:ml-0">
        ...
        <main class="flex-1 p-4 lg:p-8">
            ...
            <div class="flex items-center gap-4 mb-6">
                <a href="/organizer/events" class="flex items-center gap-2 text-muted hover:text-secondary transition-colors">
            ...
            <!-- Event Info Banner -->
            <div id="event-banner" class="bg-gradient-to-r from-primary to-primary-dark rounded-2xl p-6 mb-8 text-white relative overflow-hidden">
                <div class="absolute right-0 top-0 w-64 h-64 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2"></div>
                <div class="relative flex items-start justify-between">
                    <div>
                        <h1 id="event-title" class="text-2xl md:text-3xl font-bold mb-2">Se incarca...</h1>
                        <p id="event-info" class="text-white/80 mb-4"></p>
                        <div id="event-status" class="flex items-center gap-3"></div>
                    </div>
                    <div class="hidden lg:flex items-center gap-3">
                        <button onclick="exportCsv()" class="inline-flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl font-medium text-sm transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            Export CSV
                        </button>
                        <button onclick="exportPdf()" class="inline-flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl font-medium text-sm transition-colors">
            ...
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-2 bg-white rounded-xl p-1 border border-border">
                    <button class="period-btn px-4 py-2 text-sm font-medium bg-primary text-white rounded-lg transition-colors" data-period="7d">7 zile</button>
                    <button class="period-btn px-4 py-2 text-sm font-medium text-muted hover:bg-surface rounded-lg transition-colors" data-period="30d">30 zile</button>
                    <button class="period-btn px-4 py-2 text-sm font-medium text-muted hover:bg-surface rounded-lg transition-colors" data-period="90d">90 zile</button>
                    <button class="period-btn px-4 py-2 text-sm font-medium text-muted hover:bg-surface rounded-lg transition-colors" data-period="all">Tot</button>
                </div>
                <button onclick="refreshData()" class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-muted hover:text-secondary transition-colors">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div class="stat-card bg-white rounded-2xl p-5 border border-border">
                    ...
                    <p class="text-sm text-muted mt-1">Venituri totale</p>
                </div>
                ...
                <div class="stat-card bg-white rounded-2xl p-5 border border-border">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span id="stat-views-change" class="text-xs font-medium text-success bg-success/10 px-2 py-1 rounded-full">+0%</span>
                    </div>
                    <p id="stat-views" class="text-2xl font-bold text-secondary">0</p>
                    <p class="text-sm text-muted mt-1">Vizualizari</p>
                </div>
            ...
            <div class="grid lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white rounded-2xl border border-border p-6">
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                <h2 class="text-lg font-bold text-secondary">Performanta vanzari</h2>
                                <p class="text-sm text-muted">Venituri si bilete vandute</p>
                            </div>
                        </div>
                        <div class="h-72">
                    ...
                    <div class="bg-white rounded-2xl border border-border p-6">
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p class="text-sm text-muted">Urmareste impactul campaniilor tale</p>
                            </div>
                        ...
                        <div id="no-milestones" class="hidden text-center py-8">
                            <svg class="w-12 h-12 text-muted/30 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
                            <p class="text-muted mb-2">Nu ai campanii active</p>
                            <p class="text-sm text-muted/70">Adauga milestone-uri pentru a urmari impactul campaniilor tale</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-white rounded-2xl border border-border p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <button onclick="showAddGoalModal()" class="text-sm text-primary font-medium hover:underline">+ Adauga</button>
                        </div>
                        <div id="goals-list" class="space-y-4"></div>
                        <div id="no-goals" class="hidden text-center py-6">
                            ...
                            <p class="text-sm text-muted">Seteaza obiective pentru a urmari progresul</p>
                        </div>
                    </div>

                    <!-- Ticket Types Performance -->
                    <div class="bg-white rounded-2xl border border-border p-6">
                        <h2 class="font-bold text-secondary mb-4">Performanta tipuri bilete</h2>
                        <div id="ticket-types-list" class="space-y-3"></div>
                    </div>

                    <!-- Top Locations -->
                    <div class="bg-white rounded-2xl border border-border p-6">
                        <h2 class="font-bold text-secondary mb-4">Top locatii</h2>
                    ...
                    <div class="bg-white rounded-2xl border border-border p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="font-bold text-secondary">Vanzari recente</h2>
                            <a href="/organizer/sales?event=<?= $eventId ?>" class="text-sm text-primary font-medium">Vezi toate</a>
                        </div>
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md" onclick="event.stopPropagation()">
                <div class="p-6 border-b border-border">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-bold text-secondary">Adauga obiectiv</h2>
                        <button onclick="closeGoalModal()" class="p-2 text-muted hover:text-secondary transition-colors">
                ...
                <form id="goal-form" onsubmit="saveGoal(event)" class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-secondary mb-1">Tip obiectiv</label>
                        <select name="type" class="w-full px-4 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" required>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-secondary mb-1">Valoare tinta</label>
                        <input type="number" name="target_value" class="w-full px-4 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" required min="0" step="any">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-secondary mb-1">Deadline (optional)</label>
                        <input type="date" name="deadline" class="w-full px-4 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    </div>
                    ...
                    <div class="pt-4 flex justify-end gap-3">
                        <button type="button" onclick="closeGoalModal()" class="px-4 py-2 text-sm font-medium text-muted hover:bg-surface rounded-xl transition-colors">Anuleaza</button>
                        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-primary hover:bg-primary-dark rounded-xl transition-colors">Salveaza</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div id="milestone-modal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md" onclick="event.stopPropagation()">
                <div class="p-6 border-b border-border">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-bold text-secondary">Adauga campanie/milestone</h2>
                        <button onclick="closeMilestoneModal()" class="p-2 text-muted hover:text-secondary transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        </button>
                    </div>
                </div>
                <form id="milestone-form" onsubmit="saveMilestone(event)" class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-secondary mb-1">Nume campanie</label>
                        <input type="text" name="name" class="w-full px-4 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" required placeholder="ex: Campanie Facebook Ads">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-secondary mb-1">Tip</label>
                        <select name="type" class="w-full px-4 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" required>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-secondary mb-1">Data start</label>
                            <input type="date" name="start_date" class="w-full px-4 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-secondary mb-1">Data sfarsit</label>
                            <input type="date" name="end_date" class="w-full px-4 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-secondary mb-1">Buget (RON, optional)</label>
                        <input type="number" name="budget" class="w-full px-4 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" min="0" step="0.01">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-secondary mb-1">UTM Source (optional)</label>
                        <input type="text" name="utm_source" class="w-full px-4 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" placeholder="ex: facebook">
                    </div>
                    <div class="pt-4 flex justify-end gap-3">
                        <button type="button" onclick="closeMilestoneModal()" class="px-4 py-2 text-sm font-medium text-muted hover:bg-surface rounded-xl transition-colors">Anuleaza</button>
                        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-primary hover:bg-primary-dark rounded-xl transition-colors">Salveaza</button>
                    </div>
                </form>
            </div>
...
function updateDashboard(data) {
    ...
    if (data.event) {
        ...
        if (data.event.is_cancelled) {
            statusHtml = '<span class="px-3 py-1 bg-red-500/20 text-white rounded-full text-sm">Anulat</span>';
        } else if (data.event.is_sold_out) {
            statusHtml = '<span class="px-3 py-1 bg-amber-500/20 text-white rounded-full text-sm">Sold Out</span>';
        } else {
            statusHtml = '<span class="px-3 py-1 bg-white/20 text-white rounded-full text-sm">Activ</span>';
        }
    ...
    const listHtml = sources.map(s => `
        <div class="flex items-center justify-between py-2">
            ...
            <div class="text-right">
                ...
                <span class="text-xs text-muted ml-2">${s.visitors || 0} vizite</span>
            </div>
        </div>
    `).join('');
...
function updateMilestones(milestones) {
    ...
    const html = milestones.map(m => `
        <div class="relative pl-8 pb-4 border-l-2 border-border last:border-l-0 last:pb-0">
...
function updateTicketTypes(tickets) {
    ...
    if (!tickets || tickets.length === 0) {
        container.innerHTML = '<p class="text-sm text-muted text-center py-4">Nu exista tipuri de bilete</p>';
        return;
    }
...
function updateLocations(locations) {
    ...
    if (!locations || locations.length === 0) {
        container.innerHTML = '<p class="text-sm text-muted text-center py-4">Nu exista date</p>';
        return;
    }
    ...
    const html = locations.slice(0, 5).map((l, i) => `
        <div class="flex items-center justify-between py-2">
            <div class="flex items-center gap-2">
                <span class="w-5 h-5 bg-surface rounded text-xs font-medium flex items-center justify-center">${i + 1}</span>
...
function updateRecentSales(sales) {
    ...
    if (!sales || sales.length === 0) {
        container.innerHTML = '<p class="text-sm text-muted text-center py-4">Nu exista vanzari recente</p>';
        return;
    }
