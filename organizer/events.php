        ...
        <main class="flex-1 p-4 lg:p-8">
            ...
            <div id="events-view">
                ...
                <div id="no-events" class="hidden py-16 text-center bg-white border rounded-2xl border-border">
                    ...
                    <h2 class="mb-2 text-xl font-bold text-secondary">Nu ai evenimente încă</h2>
                    <p class="mb-6 text-muted">Creează primul tău eveniment și începe să vinzi bilete!</p>
                    <button onclick="showCreateForm()" class="btn btn-primary bg-primary"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>Creează eveniment</button>
                </div>
            </div>
                ...
                <div class="flex-1 py-2">
                    <div class="flex items-center justify-between gap-4 pr-4">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <div class="pr-4 text-right border-r border-border"><p class="text-2xl font-bold text-secondary">${event.tickets_sold || 0}</p><p class="text-xs text-muted">Bilete vândute</p></div>
                            <div class="pr-4 text-right border-r border-border"><p class="text-2xl font-bold text-secondary">${AmbiletUtils.formatCurrency(event.revenue || 0)}</p><p class="text-xs text-muted">Încasări</p></div>
                            <div class="text-right"><p class="text-2xl font-bold text-secondary">${event.views || 0}</p><p class="text-xs text-muted">Vizualizări</p></div>
                        </div>
                    </div>
...
async function loadEventForEdit(eventId) {
    ...
    try {
        ...
        AmbiletNotifications.error('Eroare la încărcarea evenimentului: ' + (error.message || 'Încearcă din nou.'));
        window.location.href = '/organizator/events';
    }
}
