...
const OrganizerPage = {
    ...
    renderOrganizer(data) {
        ...
        document.getElementById('eventsGrid').innerHTML = data.upcomingEvents.map(event => `
            <a href="/bilete/${AmbiletUtils.slugify(event.title)}" class="overflow-hidden transition-all bg-white border border-gray-200 rounded-2xl hover:-translate-y-1 hover:shadow-xl hover:border-primary group">
    ...
    renderReviews(data) {
        // TODO: API integration needed - fetch reviews from API
        const reviews = data.reviews || [];

        if (!reviews.length) {
            document.getElementById('reviewsList').innerHTML = '<p class="py-8 text-center text-gray-500">Nu sunt recenzii disponibile.</p>';
