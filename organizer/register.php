...
?>
    <header class="bg-white border-b border-border">
        <div class="flex items-center justify-between max-w-4xl px-4 py-4 mx-auto">
            <a href="/" class="flex items-center gap-2">
                <div class="flex items-center justify-center w-10 h-10 bg-primary rounded-xl"><svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/></svg></div>
    ...
    <main class="max-w-4xl px-4 py-12 mx-auto">
        <!-- Step Indicators -->
        <div class="flex items-center justify-center pb-2 mb-12 overflow-x-auto" id="step-indicators">
            <div class="flex items-center"><div id="step1-indicator" class="flex items-center justify-center w-10 h-10 text-sm font-bold text-white rounded-full bg-primary">1</div><span class="hidden ml-2 text-sm font-medium text-secondary sm:block">Cont</span></div>
            <div class="w-8 sm:w-12 h-0.5 bg-border ml-2" id="step1-line"></div>
            <div class="flex items-center"><div id="step2-indicator" class="flex items-center justify-center w-10 h-10 text-sm font-bold rounded-full bg-border text-muted">2</div><span class="hidden ml-2 text-sm text-muted sm:block">Tip</span></div>
            <div class="w-8 sm:w-12 h-0.5 bg-border ml-2" id="step2-line"></div>
            <div class="flex items-center" id="step3-container"><div id="step3-indicator" class="flex items-center justify-center w-10 h-10 text-sm font-bold rounded-full bg-border text-muted">3</div><span class="hidden ml-2 text-sm text-muted sm:block">Companie</span></div>
            <div class="w-8 sm:w-12 h-0.5 bg-border ml-2" id="step3-line"></div>
            <div class="flex items-center"><div id="step4-indicator" class="flex items-center justify-center w-10 h-10 text-sm font-bold rounded-full bg-border text-muted">4</div><span class="hidden ml-2 text-sm text-muted sm:block">Garant</span></div>
            <div class="w-8 sm:w-12 h-0.5 bg-border ml-2" id="step4-line"></div>
            <div class="flex items-center"><div id="step5-indicator" class="flex items-center justify-center w-10 h-10 text-sm font-bold rounded-full bg-border text-muted">5</div><span class="hidden ml-2 text-sm text-muted sm:block">Plata</span></div>
            <div class="w-8 sm:w-12 h-0.5 bg-border ml-2" id="step5-line"></div>
            <div class="flex items-center"><div id="step6-indicator" class="flex items-center justify-center w-10 h-10 text-sm font-bold rounded-full bg-border text-muted">6</div><span class="hidden ml-2 text-sm text-muted sm:block">Final</span></div>
        </div>
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div id="step1" class="step-content">
                <div class="p-8 bg-white border rounded-2xl border-border">
                    <h2 class="mb-2 text-2xl font-bold text-secondary">Creează contul tău</h2>
                    <p class="mb-6 text-muted">Informații pentru autentificare în platformă</p>
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <select id="phone-country" class="input w-auto min-w-[140px]" onchange="updatePhonePrefix()">
                                    <option value="RO" data-prefix="+40" selected>🇷🇴 România (+40)</option>
                                    ...
                                    <option value="FR" data-prefix="+33">🇫🇷 Franța (+33)</option>
                                    <option value="GB" data-prefix="+44">🇬🇧 Marea Britanie (+44)</option>
                                    <option value="US" data-prefix="+1">🇺🇸 SUA (+1)</option>
                                </select>
                                <div class="relative flex-1">
                                    <span id="phone-prefix" class="absolute font-medium -translate-y-1/2 left-3 top-1/2 text-muted">+40</span>
                                    <input type="tel" name="phone" id="phone-input" required class="pl-12 input" placeholder="7XX XXX XXX">
                                </div>
                            </div>
            ...
            <div id="step2" class="hidden step-content">
                <div class="p-8 bg-white border rounded-2xl border-border">
                    <h2 class="mb-2 text-2xl font-bold text-secondary">Tip Organizator</h2>
                    <p class="mb-6 text-muted">Selectează tipul de organizator și modul de lucru</p>
                    <form id="step2-form" class="space-y-6">
                        <!-- Person Type Selection -->
                        <div>
                            <label class="mb-3 label">Tip persoană *</label>
                            <div class="grid gap-4 md:grid-cols-2">
                                <label class="relative flex items-center p-4 border-2 border-border rounded-xl cursor-pointer hover:border-primary/50 transition has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-semibold text-secondary">Persoană Juridică</p>
                                    ...
                                    <div class="absolute top-3 right-3 w-5 h-5 rounded-full border-2 border-border has-[:checked]:border-primary has-[:checked]:bg-primary flex items-center justify-center">
                                        <svg class="hidden w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                    </div>
                                </label>
                                <label class="relative flex items-center p-4 border-2 border-border rounded-xl cursor-pointer hover:border-primary/50 transition has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-semibold text-secondary">Persoană Fizică</p>
                                            <p class="text-sm text-muted">Fără formă juridică</p>
                                        </div>
                                    </div>
                                    <div class="absolute flex items-center justify-center w-5 h-5 border-2 rounded-full top-3 right-3 border-border"></div>
                                </label>
                            </div>
                        </div>
                        ...
                        <!-- Work Mode Selection -->
                        <div>
                            <label class="mb-3 label">Mod de lucru *</label>
                            <div class="grid gap-4 md:grid-cols-2">
                                <label class="relative flex items-start p-4 border-2 border-border rounded-xl cursor-pointer hover:border-primary/50 transition has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm text-muted">Vinzi bilete doar prin <?= SITE_NAME ?>. Beneficiezi de comision redus și invitații gratuite.</p>
                                    </div>
                                    <div class="absolute flex items-center justify-center w-5 h-5 border-2 rounded-full top-3 right-3 border-border"></div>
                                </label>
                                <label class="relative flex items-start p-4 border-2 border-border rounded-xl cursor-pointer hover:border-primary/50 transition has-[:checked]:border-primary has-[:checked]:bg-primary/5">
            ...
            <div id="step3" class="hidden step-content">
                <div class="p-8 bg-white border rounded-2xl border-border">
                    <h2 class="mb-2 text-2xl font-bold text-secondary">Detalii Companie</h2>
                    <p class="mb-6 text-muted">Verifică datele companiei tale la ANAF</p>
                    <form id="step3-form" class="space-y-6">
                        <!-- CUI Verification -->
                        <div>
                            <label class="label">CUI / CIF *</label>
                            <div class="flex gap-2">
                                <input type="text" name="cui" id="cui-input" class="flex-1 input" placeholder="12345678 sau RO12345678" required>
                            ...
                            <p class="mt-1 text-xs text-muted">Introdu CUI-ul și apasă verificare pentru a prelua datele automat</p>
                        </div>
                        ...
                        <div id="company-details" class="hidden space-y-4">
                            ...
                            <div class="grid gap-4 md:grid-cols-2">
                                <div>
                                    <label class="label">Nr. Reg. Comerțului</label>
                                ...
                                <div>
                                    ...
                                    <div id="vat-display" class="cursor-not-allowed input bg-gray-50">-</div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="organizer_type" id="organizer-type-pj" class="input" required>
                                    ...
                                    <option value="agency">Agenție de evenimente</option>
                                    <option value="promoter">Promoter independent</option>
                                    <option value="venue">Locație / Sală</option>
                                    <option value="artist">Artist / Manager</option>
                                    <option value="ngo">ONG / Fundație</option>
                            ...
                            <div class="pt-4 border-t border-border">
                                <h3 class="mb-4 text-lg font-semibold text-secondary">Reprezentant Legal</h3>
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div>
            ...
            <div id="step4" class="hidden step-content">
                <div class="p-8 bg-white border rounded-2xl border-border">
                    <h2 class="mb-2 text-2xl font-bold text-secondary" id="step4-title">Date Personale</h2>
                    <p class="mb-6 text-muted" id="step4-subtitle">Informații pentru identificare și contract</p>
                    <form id="step4-form" class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-muted">Codul Numeric Personal - 13 cifre</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="guarantor_city" id="guarantor-city" required class="input" placeholder="București">
                        </div>

                        <!-- ID Card Details -->
                        <div class="pt-4 border-t border-border">
                            <h3 class="mb-4 text-lg font-semibold text-secondary">Act de Identitate</h3>
                            <div class="grid gap-4 md:grid-cols-2">
                                <div>
            ...
            <div id="step5" class="hidden step-content">
                <div class="p-8 bg-white border rounded-2xl border-border">
                    <h2 class="mb-2 text-2xl font-bold text-secondary">Detalii Bancare</h2>
                    <p class="mb-6 text-muted">Contul în care vei primi plățile pentru biletele vândute</p>
                    <form id="step5-form" class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-muted">Numele exact al titularului contului bancar</p>
                        </div>

                        <div>
                            <label class="label">Banca</label>
                            <input type="text" name="bank_name" id="bank-name" class="input" placeholder="Se completează automat" readonly>
                        </div>
                        ...
                        <div class="p-4 bg-accent/10 rounded-xl">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-secondary">Când primesc banii?</p>
                                    <p class="text-sm text-muted">Plățile sunt procesate în maxim 5 zile lucrătoare după eveniment. Pentru evenimente cu valoare mare, poți solicita plăți parțiale în avans.</p>
                                </div>
                            </div>
                        </div>
            ...
            <div id="step6" class="hidden step-content">
                <div class="p-8 bg-white border rounded-2xl border-border">
                    <h2 class="mb-2 text-2xl font-bold text-secondary">Finalizare Înregistrare</h2>
                    <p class="mb-6 text-muted">Verifică datele și confirmă înregistrarea</p>

                    <!-- Summary Sections -->
                    <div class="mb-6 space-y-4">
                        <!-- Account Summary -->
                        <div class="p-4 bg-surface rounded-xl">
                            <h4 class="flex items-center gap-2 mb-2 font-medium text-secondary">
                            ...
                            <div class="space-y-1 text-sm text-muted" id="summary-account"></div>
                        </div>
                        ...
                        <div class="p-4 bg-surface rounded-xl">
                            ...
                            <div class="space-y-1 text-sm text-muted" id="summary-type"></div>
                        </div>
                        ...
                        <div class="p-4 bg-surface rounded-xl" id="summary-company-section">
                            ...
                            <div class="space-y-1 text-sm text-muted" id="summary-company"></div>
                        </div>
                        ...
                        <div class="p-4 bg-surface rounded-xl">
                            ...
                            <div class="space-y-1 text-sm text-muted" id="summary-guarantor"></div>
                        </div>
                        ...
                        <div class="p-4 bg-surface rounded-xl">
                            ...
                            <div class="space-y-1 text-sm text-muted" id="summary-bank"></div>
                        </div>
                    </div>

                    <!-- Commission Info -->
                    <div class="p-4 mb-6 space-y-3 border bg-primary/5 border-primary/20 rounded-xl">
                        <h4 class="font-semibold text-secondary">Informații Tarifare</h4>
                        <div class="space-y-2 text-sm">
                            <div class="flex items-start gap-2">
                                ...
                                <span><strong>Comision:</strong> maxim 6% din valoarea biletului, dar nu mai puțin de 2.50 lei per bilet emis</span>
                            </div>
                            <div class="flex items-start gap-2" id="invitation-cost-info">
                                <svg class="w-4 h-4 text-primary flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                <span><strong>Cost invitație:</strong> <span id="invitation-cost-text">0.25 lei per bucată</span></span>
                            </div>
                        </div>
                    </div>

                    <!-- Contract Info -->
                    <div class="p-4 mb-6 border bg-accent/10 border-accent/20 rounded-xl">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-muted">Prin finalizarea înregistrării, îți vom genera un contract electronic personalizat. Contractul va trebui semnat (electronic sau olograf) și încărcat în platformă pentru validarea contului.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-6 space-y-3">
                        <label class="flex items-start gap-3 cursor-pointer">
                            <input type="checkbox" name="terms" required class="w-4 h-4 mt-1 rounded border-border text-primary focus:ring-primary">
                            <span class="text-sm text-muted">Accept <a href="/terms.php" class="text-primary hover:underline" target="_blank">Termenii și Condițiile</a> pentru organizatori si <a href="/privacy.php" class="text-primary hover:underline" target="_blank">Politica de Confidențialitate</a> *</span>
                        </label>
                        <label class="flex items-start gap-3 cursor-pointer">
                            <input type="checkbox" name="contract" required class="w-4 h-4 mt-1 rounded border-border text-primary focus:ring-primary">
                            <span class="text-sm text-muted">Sunt de acord cu generarea <a href="/contract.php" class="text-primary hover:underline" target="_blank">Contractului de prestări servicii</a> <?= SITE_NAME ?> *</span>
                        </label>
                    </div>
...
document.getElementById('step2-form').addEventListener('submit', (e) => {
    ...
    if (!personType) {
        showError('Selectează tipul de persoană.');
        return;
    }

    if (!workMode) {
        showError('Selectează modul de lucru.');
        return;
    }
...
document.getElementById('step3-form').addEventListener('submit', (e) => {
    ...
    if (!formData.anaf_verified) {
        showError('Trebuie să verifici CUI-ul la ANAF.');
        return;
    }
    ...
    if (!organizerType) {
        showError('Selectează tipul de organizator.');
        return;
    }
...
function updateStep4ForPersonType(personType) {
    ...
    if (personType === 'pf') {
        ...
        subtitle.textContent = 'Informații pentru identificare și contract';
        ...
        subtitle.textContent = 'Persoana care garantează pentru companie';
...
async function verifyCUI() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<div class="w-5 h-5 spinner"></div> Verificare...';
    ...
    try {
        ...
        if (response.success && response.data) {
            ...
            if (isVatPayer) {
                ...
                vatBadge.textContent = 'Plătitor TVA';
            } else {
            ...
            AmbiletNotifications.error(response.message || 'CUI invalid sau nu a fost găsit');
            formData.anaf_verified = false;
        }
    } catch (error) {
        console.error('ANAF verification error:', error);
        AmbiletNotifications.error('Eroare la verificarea ANAF. Încearcă din nou.');
        formData.anaf_verified = false;
    }
...
function validateIBAN(input) {
    ...
    if (value.length > 24) {
        validation.textContent = 'IBAN-ul românesc are exact 24 caractere';
        validation.className = 'mt-1 text-xs text-red-600';
        input.classList.add('border-red-500');
        input.classList.remove('border-green-500');
        bankNameInput.value = '';
...
function buildSummary() {
    ...
    document.getElementById('summary-type').innerHTML = `
        <p><strong>Tip persoană:</strong> ${personTypeLabel}</p>
    ...
    if (formData.person_type === 'pj') {
        ...
        document.getElementById('summary-company').innerHTML = `
            ...
            <p><strong>Plătitor TVA:</strong> ${formData.vat_payer ? 'Da' : 'Nu'}</p>
    ...
    if (formData.work_mode === 'exclusive') {
        ...
        invitationCostText.textContent = '0.25 lei per bucată';
    }
}
...
async function submitRegistration() {
    ...
    if (!termsChecked || !contractChecked) {
        AmbiletNotifications.error('Trebuie să accepți termenii și contractul');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            showError(result.message || 'Eroare la înregistrare.');
            btn.disabled = false;
        ...
        AmbiletNotifications.error('A apărut o eroare. Încearcă din nou.');
        btn.disabled = false;
        btnText.classList.remove('hidden');
        btnSpinner.classList.add('hidden');
    }
