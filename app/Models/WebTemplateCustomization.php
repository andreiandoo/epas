...
class WebTemplateCustomization extends Model
{
    ...
    public function setPreviewPasswordAttribute(?string $value): void
    {
        // Hash the password on save; skip if already hashed or empty
        if ($value && !str_starts_with($value, '$2y$') && !str_starts_with($value, '$2a$')) {
            $this->attributes['preview_password'] = Hash::make($value);
        } else {
            $this->attributes['preview_password'] = $value;
        }
    }
    ...
    {
        return Hash::check($password, $this->preview_password);
    }
