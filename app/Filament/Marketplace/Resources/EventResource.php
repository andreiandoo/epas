...
class EventResource extends Resource
{
    ...
    /**
     * True when the logged-in marketplace user is an Administrator or Super
     * Administrator (not a Moderator). Gates the event-statistics section.
     */
    protected static function isMarketplaceAdminUser(): bool
    {
        return in_array(
            \Illuminate\Support\Facades\Auth::guard('marketplace_admin')->user()?->role,
            ['super_admin', 'admin'],
            true
        );
    }
    ...
    /**
     * Per-event sales numbers (tickets / revenue / commission, split online vs
     * POS, per-day averages, and today) via SalesBreakdownService. Cached 5 min
     * so Livewire re-renders don't re-run four breakdown builds each time.
     */
    protected static function computeEventStats(Event $event): array
    {
        return \Illuminate\Support\Facades\Cache::remember(
            "mp_event_stats_{$event->id}",
            300,
            function () use ($event) {
                $svc = app(\App\Services\Marketplace\SalesBreakdownService::class);
                $tz = 'Europe/Bucharest';
                $todayStart = \Illuminate\Support\Carbon::now($tz)->startOfDay()->utc();
                $todayEnd = \Illuminate\Support\Carbon::now($tz)->endOfDay()->utc();

                $tk = fn (array $bd) => (int) collect($bd['per_type'] ?? [])->sum('valid_count');
                ...
                // Days on sale = from the first valid/used ticket to today.
                $firstSale = \App\Models\Ticket::where(function ($q) use ($event) {
                        $q->where('event_id', $event->id)->orWhere('marketplace_event_id', $event->id);
                    })
                    ->whereIn('status', ['valid', 'used'])
                    ->min('created_at');
                $days = $firstSale
                    ? max(1, \Illuminate\Support\Carbon::parse($firstSale)->timezone($tz)->startOfDay()->diffInDays(\Illuminate\Support\Carbon::now($tz)->startOfDay()) + 1)
                    : 1;
    ...
    protected static function renderEventStats(Event $event): \Illuminate\Support\HtmlString
    {
        $d = static::computeEventStats($event);
        $cur = $d['currency'];
        $money = fn ($v) => number_format((float) $v, 2, ',', '.') . ' ' . $cur;
        $num = fn ($v) => number_format((float) $v, 0, ',', '.');
        $avg2 = fn ($v) => number_format((float) $v, 1, ',', '.');
        ...
        $split = fn ($on, $po, $fmt) => 'online: ' . $fmt($on) . ' · POS: ' . $fmt($po);

        $html = '<div class="space-y-3">';
        // Totaluri
        $html .= '<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">';
        $html .= $tile('Total bilete', $num($d['tickets_total']), $split($d['tickets_online'], $d['tickets_pos'], $num), 'indigo');
        $html .= $tile('Total vânzări', $money($d['sales_total']), $split($d['sales_online'], $d['sales_pos'], $money), 'emerald');
        $html .= $tile('Total comisioane', $money($d['comm_total']), $split($d['comm_online'], $d['comm_pos'], $money), 'amber');
        $html .= '</div>';
        // Medii pe zi
        $html .= '<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">';
        $html .= $tile('Bilete / zi', $avg2($d['tickets_total'] / $d['days']), $d['days'] . ' zile de vânzare', 'purple');
        $html .= $tile('Încasări / zi', $money($d['sales_total'] / $d['days']), '', 'purple');
        $html .= $tile('Comision / zi', $money($d['comm_total'] / $d['days']), '', 'purple');
        $html .= '</div>';
        // Azi
        $html .= '<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">';
        $html .= $tile('Bilete azi', $num($d['tickets_today']), '', 'blue');
        $html .= $tile('Vânzări azi', $money($d['sales_today']), '', 'blue');
        $html .= $tile('Comision azi', $money($d['comm_today']), '', 'blue');
        $html .= '</div>';
        $html .= '</div>';
    ...
    public static function form(Schema $schema): Schema
    {
        ...
        return $schema->schema([
            ...
            // Event statistics — admin / super-admin only. Tickets, sales &
            // commission (online vs POS), per-day averages, and today.
            SC\Section::make($t('Statistici eveniment', 'Event statistics'))
                ->icon('heroicon-o-chart-bar')
                ->collapsible()
                ->collapsed()
                ->persistCollapsed()
                ->visible(fn ($record) => $record !== null && static::isMarketplaceAdminUser())
                ->schema([
                    Forms\Components\Placeholder::make('event_stats')
                        ->hiddenLabel()
                        ->content(fn ($record) => $record
                            ? static::renderEventStats($record)
                            : new HtmlString('')),
                ]),
                        ...
                        SC\Section::make($t('Taxonomii și Relații', 'Taxonomies & Relations'))
                            ->schema([
                                ...
                                SC\Tabs\Tab::make($t('Configurare Locație', 'Venue Config'))
                                    ...
                                    ->lazy()
                                    ->schema([
                                        // Schema venue-config (Sezoane + General + Pricing + Hero + Atractii + Trasee
                                        // + Flora + Galerie + FAQ etc.) consuma peste 512MB la CONSTRUIREA Schema-ului
                                        // Filament 4 pentru evenimentul Lacul Sf. Ana — randarea Blade ulterioara aduce
                                        // depasirea. Editarea acestui continut e oricum facuta de operator prin
                                        // /organizator/leisure tab Setari (interfata dedicata, mai rapida si fara
                                    ...
                                    ->schema([
                                        ...
                                        SC\Section::make($t('Identificare', 'Identification'))
                                            ...
                                            ->schema([
                                                ...
                                                Forms\Components\Placeholder::make('slots_section_header')
                                                    ->hiddenLabel()
                                                    ->content(new HtmlString('<div style="border-top:1px solid #e5e7eb;padding-top:12px;margin-top:8px"><div style="font-weight:600;font-size:13px;color:#374151">🕐 Slot-uri pe oră (booking pe interval)</div><div style="font-size:11px;color:#6b7280;margin-top:2px">Pentru produse cu curse repetitive (vaporașe, ghidaj).</div></div>'))
                                                    ->visible(fn (SGet $get) => ($get('../../display_template') ?? 'standard') === 'leisure_venue'
                                                        && in_array(($get('service_category') ?: 'access'), ['rental', 'activity']))
                                                    ->columnSpan(12),
                                                ...
                                                Forms\Components\TextInput::make('meta.slots_config.first_slot')
                                                    ...
                                                    ->visible(fn (SGet $get) => ($get('../../display_template') ?? 'standard') === 'leisure_venue'
                                                        && in_array(($get('service_category') ?: 'access'), ['rental', 'activity'])
                                                        && (bool) $get('meta.slots_config.enabled'))
                                                    ->columnSpan(3),
                                                ...
                                                Forms\Components\TextInput::make('meta.slots_config.interval_minutes')
                                                    ->label($t('Interval (min)', 'Interval (min)'))
                                                    ->numeric()
                                                    ->minValue(5)
                                                    ->default(30)
                                                    ->visible(fn (SGet $get) => ($get('../../display_template') ?? 'standard') === 'leisure_venue'
                                                ...
                                                // ── F5: Inventar fizic (lock pe interval — Bărci etc.) ──
                                                Forms\Components\Placeholder::make('physical_inventory_section_header')
                                                    ->hiddenLabel()
                                                    ->content(new HtmlString('<div style="border-top:1px solid #e5e7eb;padding-top:12px;margin-top:8px"><div style="font-weight:600;font-size:13px;color:#374151">🔒 Inventar fizic (lock pe interval)</div><div style="font-size:11px;color:#6b7280;margin-top:2px">Pentru unități fizice (bărci, kayak-uri). O unitate nu poate fi rezervată în intervale care se suprapun.</div></div>'))
                                                    ->visible(fn (SGet $get) => ($get('../../display_template') ?? 'standard') === 'leisure_venue'
                                                        && in_array(($get('service_category') ?: 'access'), ['rental', 'activity']))
                                                    ->columnSpan(12),
                                                ...
                                                Forms\Components\TextInput::make('meta.physical_inventory.count')
                                                    ...
                                                    ->visible(fn (SGet $get) => ($get('../../display_template') ?? 'standard') === 'leisure_venue'
                                                        && in_array(($get('service_category') ?: 'access'), ['rental', 'activity'])
                                                        && (bool) $get('meta.physical_inventory.enabled'))
                                                    ->columnSpan(12),
    ...
    public static function getSalesBreakdown(Event $event): array
    {
        ...
        // Online-only build (excludes pos_app + test_order) lets us split
        // revenue/commission into "what flowed through the marketplace" vs
        // "what the organizer collected at the door". POS commission is
        // still real commission — it just gets invoiced separately and
        // never lands in a payout decont. Two cheap reads of the same
        // tables; cached on the resource for the request's lifetime.
        $onlineBreakdown = $service->build($event, null, null, excludePos: true);
