                        ...
                        SC\Section::make($t('Taxonomii și Relații', 'Taxonomies & Relations'))
                            ->schema([
                                ...
                                Forms\Components\Select::make('artists')
                                    ...
                                    ->multiple()
                                    ->searchable()
                                    ->getSearchResultsUsing(fn (string $search) => \App\Models\Artist::query()
                                        ->whereRaw('unaccent(LOWER(name)) LIKE unaccent(?)', ['%' . mb_strtolower($search) . '%'])
                                        ->limit(50)
                                        ->pluck('name', 'id')
                                        ->toArray())
                                    ->getOptionLabelsUsing(fn (array $values) => \App\Models\Artist::whereIn('id', $values)->pluck('name', 'id')->toArray())
                                    ->live(onBlur: true)
                                    ->partiallyRenderAfterStateUpdated()
                                    ->suffixAction(
