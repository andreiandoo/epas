...
class CreateTenant extends CreateRecord
{
    ...
    protected function sendVenueOwnerWelcomeEmail(int $mcId, string $email, string $fullName, string $publicName): void
    {
        ...
        $html = '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head><body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,sans-serif;background:#f8fafc">'
            ...
            . '<div style="margin:8px 0 28px;padding:24px;border-radius:14px;background-color:#0f172a;background-image:linear-gradient(135deg,#0f172a 0%,#1e3a8a 100%);color:#ffffff">'
            . '<p style="margin:0 0 6px;font-size:12px;letter-spacing:1px;text-transform:uppercase;color:#93c5fd;font-weight:700">Pas 2 — aplicația AmBilet pentru Android</p>'
            . '<p style="margin:0 0 18px;font-size:19px;line-height:1.35;font-weight:700;color:#ffffff">Descarcă aplicația pe telefon: scanezi bilete și vinzi la intrare</p>'
            . '<div style="text-align:center;margin:0 0 20px">'
            . '<a href="' . htmlspecialchars($appUrl) . '" style="display:inline-block;background:#22c55e;color:#052e16;text-decoration:none;padding:15px 32px;border-radius:10px;font-weight:700;font-size:17px">Descarcă aplicația Android (APK)</a>'
            . '</div>'
            . '<p style="margin:0 0 8px;font-size:14px;font-weight:700;color:#ffffff">Cum o instalezi:</p>'
            . '<ol style="margin:0;padding-left:20px;font-size:14px;line-height:1.8;color:#e2e8f0">'
            . '<li>Deschide linkul de mai sus de pe telefonul Android. Se descarcă fișierul aplicației (APK).</li>'
            . '<li>Deschide fișierul descărcat. Dacă telefonul îți cere, permite instalarea din această sursă („surse necunoscute”), apoi apasă Instalează.</li>'
            . '<li>Deschide aplicația AmBilet și autentifică-te cu emailul <strong style="color:#ffffff">' . htmlspecialchars($email) . '</strong> și parola setată la Pas 1.</li>'
            . '</ol>'
            . '<p style="margin:16px 0 0;font-size:12px;color:#94a3b8">Aplicația nu este încă în Google Play, se instalează direct din link. Momentan există doar varianta pentru Android.</p>'
            . '</div>'
