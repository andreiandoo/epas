...
class PlatformAudienceResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema
            ->schema([
                Components\Section::make('Audience Details')
                    ->description('Define your marketing audience')
                    ->schema([
                        Components\Select::make('platform_ad_account_id')
                            ->label('Ad Account')
                        ...
                        Components\Select::make('audience_type')
                            ->label('Audience Type')
                ...
                Components\Section::make('Sync Settings')
                    ->schema([
                        Components\Toggle::make('is_auto_sync')
                            ->label('Auto-Sync')
                            ->helperText('Automatically sync this audience to the ad platform')
                            ->default(false)
                            ->live(),

                        Components\Select::make('sync_frequency')
                        ...
                        Components\TextInput::make('platform_audience_id')
                ...
                Components\Section::make('Lookalike Configuration')
                    ->description('Configure the source and targeting for your lookalike audience')
                    ->schema([
                        Components\Select::make('lookalike_source_type')
                            ->label('Seed Source')
                        ...
                        Components\Select::make('lookalike_source_audience_id')
                        ...
                        Components\Select::make('lookalike_percentage')
                            ->label('Audience Size')
                        ...
                        Components\Select::make('lookalike_country')
                        ...
                        Components\Placeholder::make('lookalike_preview')
                ...
                Components\Section::make('Custom Segment Rules')
                    ->description('Define custom rules for this audience')
                    ->schema([
                        Components\Repeater::make('segment_rules')
                            ->label('Rules')
                            ->schema([
                                Components\Select::make('field')
                                    ->label('Field')
                                    ->options([
                                ...
                                Components\Select::make('operator')
                                    ->label('Operator')
                                    ->options([
                                        '=' => 'Equals',
                                        '!=' => 'Not Equals',
                ...
                Components\Section::make('Statistics')
                    ->schema([
                        Components\Placeholder::make('member_count_display')
                            ->label('Members')
                            ->content(fn ($record) => number_format($record?->member_count ?? 0)),

                        Components\Placeholder::make('matched_count_display')
                            ->label('Matched')
                            ->content(fn ($record) => number_format($record?->matched_count ?? 0)),

                        Components\Placeholder::make('match_rate_display')
                            ->label('Match Rate')
                            ->content(fn ($record) => ($record?->getMatchRate() ?? 0) . '%'),

                        Components\Placeholder::make('last_synced_display')
                            ->label('Last Synced')
                ...
                Action::make('create_lookalike')
                    ...
                    ->form([
                        Components\Select::make('platform_ad_account_id')
                            ...
                            ->required(),
                        Components\Select::make('percentage')
                            ->label('Audience Size')
                            ->options(PlatformAudience::getLookalikePercentageOptions())
                            ->default(1)
                            ->required(),
