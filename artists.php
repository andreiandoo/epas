...
const ArtistsPage = {
    ...
    renderTrendingItem(artist, rank) {
        ...
        return \`
            <a href="/artist/\${artist.slug}" class="flex items-center gap-4 p-4 transition-colors rounded-2xl bg-surface hover:bg-border/50">
                ...
                <div class="flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-md \${changeClass}">
                    \${changeIcon}
                    \${artist.change >= 0 ? '+' : ''}\${artist.change || 0}%
                </div>
            </a>
        \`;
    },
