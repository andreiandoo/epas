...
export class Router {
    ...
    private setupDefaultRoutes(): void {
        ...
        this.addRoute('/account/tickets/:id', this.renderTicketDetail.bind(this));
            ...
            if (ticketsListEl) {
                if (tickets && tickets.length > 0) {
                    ticketsListEl.innerHTML = tickets.map((ticket: any) => `
                        <div class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition cursor-pointer ticket-card" data-ticket-id="${ticket.id}" data-ticket-code="${ticket.code}" data-event-name="${ticket.event_name}" data-event-date="${ticket.date}" data-venue="${ticket.venue || ''}" data-qr-code="${ticket.qr_code}">
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex-1">
    ...
    private showTicketDetailModal(ticketData: any, fullTicket: any): void {
        // Create modal overlay
        const modalOverlay = document.createElement('div');
        modalOverlay.id = 'ticket-modal-overlay';
        modalOverlay.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4';
        modalOverlay.style.display = 'flex';

        const statusClass =
            fullTicket.status === 'valid' || fullTicket.status === 'pending' ? 'bg-green-100 text-green-800' :
            fullTicket.status === 'used' ? 'bg-blue-100 text-blue-800' :
            fullTicket.status === 'cancelled' ? 'bg-red-100 text-red-800' :
            'bg-gray-100 text-gray-800';

        modalOverlay.innerHTML = `
            <div class="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
                <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                    <h2 class="text-2xl font-bold text-gray-900">Detalii Bilet</h2>
                ...
                <div class="p-6 space-y-6">
                    <!-- Event Countdown -->
                    ${ticketData.date ? `
                    <div id="ticket-countdown" class="bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg p-4 shadow-lg">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                                <span class="font-medium">Timp rămas până la eveniment:</span>
                            </div>
                            <div class="countdown-display text-lg font-bold">
                                <span class="countdown-days">--</span>z
                                <span class="countdown-hours">--</span>h
                    ...
                    <div class="flex justify-center">
                        <span class="inline-block px-4 py-2 text-sm font-medium rounded-full ${statusClass}">
                            ${fullTicket.status_label}
                        </span>
                    </div>

                    <!-- QR Code -->
                    <div class="flex flex-col items-center space-y-3 bg-gray-50 rounded-lg p-6">
                        <img src="${ticketData.qr_code}" alt="QR Code" class="w-64 h-64 border-4 border-white shadow-lg rounded-lg">
                        <div class="text-center">
                            <p class="text-sm text-gray-600 mb-1">Cod bilet:</p>
                            <p class="text-xl font-mono font-bold text-gray-900">${ticketData.code}</p>
                        </div>
                    </div>

                    <!-- Event Information -->
                    <div class="bg-gray-50 rounded-lg p-4 space-y-3">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3">Informații Eveniment</h3>
                        <div class="space-y-2">
                            <div class="flex items-start">
                                <svg class="w-5 h-5 mr-2 mt-0.5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/>
                                </svg>
                                <div>
                                    <p class="text-sm font-medium text-gray-900">${ticketData.event_name}</p>
                                    <p class="text-xs text-gray-600">${fullTicket.ticket_type}</p>
                                </div>
                            </div>
                            ${ticketData.date ? `
                            <div class="flex items-center">
                                ...
                                <p class="text-sm text-gray-700">${new Date(ticketData.date).toLocaleDateString('ro-RO', {weekday: 'long', day: 'numeric', month: 'long', year: 'numeric'})}</p>
                            </div>
                            ` : ''}
                            ${ticketData.venue ? `
                            <div class="flex items-center">
                                ...
                                <p class="text-sm text-gray-700">${ticketData.venue}</p>
                            </div>
                            ` : ''}
                            ${fullTicket.seat_label ? `
                            <div class="flex items-center">
                                <svg class="w-5 h-5 mr-2 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9"/>
                                </svg>
                                <p class="text-sm text-gray-700">Loc: ${fullTicket.seat_label}</p>
                            </div>
                            ` : ''}
                        </div>
                    </div>
                    ...
                    ${fullTicket.beneficiary ? `
                    <div class="bg-blue-50 rounded-lg p-4">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3">Beneficiar</h3>
                        <div class="space-y-1">
                            <p class="text-sm"><span class="font-medium">Nume:</span> ${fullTicket.beneficiary.name}</p>
                            ${fullTicket.beneficiary.email ? `<p class="text-sm"><span class="font-medium">Email:</span> ${fullTicket.beneficiary.email}</p>` : ''}
                            ${fullTicket.beneficiary.phone ? `<p class="text-sm"><span class="font-medium">Telefon:</span> ${fullTicket.beneficiary.phone}</p>` : ''}
                        </div>
                    </div>
                    ` : ''}
                    ...
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <p class="text-sm text-yellow-800">
                            <strong>Important:</strong> Prezentă acest cod QR la intrare pentru scanare.
                            Recomandăm salvarea unei capturi de ecran pentru acces offline.
                        </p>
                    </div>
                </div>
            </div>
        ...
        // Initialize countdown if event has date
        if (ticketData.date) {
            const countdownEl = document.getElementById('ticket-countdown');
            if (countdownEl) {
                const updateCountdown = () => {
                    const eventDateTime = new Date(ticketData.date);
                    const now = new Date();
                    const diff = eventDateTime.getTime() - now.getTime();
