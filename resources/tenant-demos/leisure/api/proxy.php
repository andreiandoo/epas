<?php
/**
 * Proxy browser → core.tixello.com pentru API-ul public (nordvale, tenant leisure).
 *
 * Evită CORS (apel server-side) și menține o sesiune stabilă de hold-uri
 * (X-Session-Id) per vizitator, printr-un cookie propriu. Rutează și
 * auth-ul, contul, coșul/checkout-ul și abonamentele către tenant-client.
 *
 * Acțiuni principale:
 ...
 *   POST ?action=checkout  → demo-checkout (creează comanda)
 *   POST ?action=login / register / logout, GET ?action=me
 *   GET/POST ?action=acc-*  → contul clientului
 */
...
switch ($action) {
    ...
    case 'forgot-password':
        $method = 'POST';
        $path   = '/tenant-client/auth/forgot-password';
        $query  = ['tenant' => TENANT_ID, 'hostname' => ($_SERVER['HTTP_HOST'] ?? '')];
        $body   = json_decode(file_get_contents('php://input'), true) ?: [];
        break;
    ...
    case 'reset-password':
        $method = 'POST';
        $path   = '/tenant-client/auth/reset-password';
        $query  = ['tenant' => TENANT_ID, 'hostname' => ($_SERVER['HTTP_HOST'] ?? '')];
        $body   = json_decode(file_get_contents('php://input'), true) ?: [];
        break;
    ...
    case 'acc-refund-request':
        $method = 'POST';
        $path   = '/tenant-client/account/refund-request';
        $query  = ['tenant' => TENANT_ID];
        $body   = json_decode(file_get_contents('php://input'), true) ?: [];
        break;

    case 'voucher-verify':
        $path   = '/tenant-client/voucher/' . rawurlencode($_GET['code'] ?? '');
        $query  = ['tenant' => TENANT_ID];
        break;
...
// Forward token-ul de autentificare al clientului (pt. /me, /logout, acc-*)
...
curl_setopt_array($ch, [
    ...
    CURLOPT_USERAGENT      => 'nordvale-skin-proxy/1.0',
]);
if ($body !== null) {
