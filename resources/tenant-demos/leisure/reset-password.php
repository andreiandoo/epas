...
<head>
  ...
  <title>Setează o parolă nouă — Nordvale</title>
  <meta name="description" content="Setează o parolă nouă pentru contul Nordvale.">
  ...
  <style>
    ...
    *{box-sizing:border-box} html{background:var(--oat)}
    ...
    .token-status{display:flex;align-items:flex-start;gap:10px;margin:18px 0 0;padding:12px 13px;border:1px solid rgba(27,82,66,.14);border-radius:16px;background:rgba(223,252,98,.13)}
    .token-status svg{flex:0 0 auto;color:var(--pine-700);margin-top:1px}.token-status strong{display:block;font-size:12px}.token-status span{display:block;margin-top:3px;color:rgba(22,35,30,.56);font-size:11px;line-height:1.45}
    .toggle-pass{position:absolute;right:10px;top:50%;width:36px;height:36px;transform:translateY(-50%);display:grid;place-items:center;border:0;border-radius:11px;background:transparent;color:rgba(22,35,30,.48);cursor:pointer}.toggle-pass:hover{background:rgba(9,37,29,.06);color:var(--pine-700)}
    .input.password{padding-right:54px}.strength{display:grid;gap:8px;margin-top:2px}.strength-bars{display:grid;grid-template-columns:repeat(4,1fr);gap:5px}.strength-bars span{height:5px;border-radius:999px;background:rgba(9,37,29,.09);transition:.22s ease}.strength[data-level="1"] .strength-bars span:nth-child(1),.strength[data-level="2"] .strength-bars span:nth-child(-n+2),.strength[data-level="3"] .strength-bars span:nth-child(-n+3),.strength[data-level="4"] .strength-bars span{background:var(--acid)}.strength[data-level="1"] .strength-bars span:nth-child(1){background:#d9674f}.strength[data-level="2"] .strength-bars span:nth-child(-n+2){background:#e8a34c}.strength-label{display:flex;justify-content:space-between;gap:10px;color:rgba(22,35,30,.5);font-size:10px;font-weight:700}.rules{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:2px}.rule{display:flex;align-items:center;gap:7px;color:rgba(22,35,30,.52);font-size:10px;font-weight:700}.rule i{width:17px;height:17px;display:grid;place-items:center;border-radius:50%;background:rgba(9,37,29,.07);font-style:normal;font-size:10px}.rule.ok{color:var(--pine-700)}.rule.ok i{background:var(--acid);color:var(--pine-950)}
    .success-badge{display:inline-flex;align-items:center;gap:7px;margin-top:16px;padding:8px 11px;border-radius:999px;background:rgba(223,252,98,.22);color:var(--pine-700);font-size:10px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}
    @media(max-width:420px){.rules{grid-template-columns:1fr}.token-status{padding:11px}.form{gap:14px}}
</style>
</head>
<body>
<div class="page">
  <header class="header-wrap">
    <div class="header">
      <a class="brand" href="/"><span class="brand-mark"><svg viewBox="0 0 48 48" width="30" height="30" fill="none"><path d="M7 35 18 11l7 14 5-10 11 20H7Z" fill="#DFFC62"/><path d="m8 37 11-10 6 6 7-8 8 12H8Z" fill="#FFFDF6"/></svg></span><span><span class="brand-name">Nordvale</span><span class="brand-sub">Adventure reserve</span></span></a>
  ...
  <aside class="mobile-menu" id="mobileMenu" aria-hidden="true">
    <div class="mobile-menu-head"><a class="brand" href="/"><span class="brand-mark"><svg viewBox="0 0 48 48" width="30" height="30" fill="none"><path d="M7 35 18 11l7 14 5-10 11 20H7Z" fill="#DFFC62"/><path d="m8 37 11-10 6 6 7-8 8 12H8Z" fill="#FFFDF6"/></svg></span><span class="brand-name">Nordvale</span></a><button class="menu-btn" id="menuClose" type="button" aria-label="Închide meniul"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 6 12 12M18 6 6 18"/></svg></button></div>
    <div class="mobile-menu-body"><nav><a href="/">Acasă</a><a href="/experiente">Experiențe</a><a href="/planifica">Planifică vizita</a><a href="/autentificare">Autentificare</a></nav><p style="color:rgba(255,255,255,.45);font-size:12px;line-height:1.6">Ai nevoie de ajutor?<br><strong style="color:white">support@nordvale.demo</strong></p></div>
  </aside>

  <main class="main">
    <section class="visual topo" aria-label="Securitate cont Nordvale">
      <div class="visual-bg"><img src="https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=1800&q=86" alt="Pădure montană luminată la răsărit"></div>
      ...
      <div class="visual-content">
        <div class="eyebrow"><span class="eyebrow-dot"></span>Ultimul pas</div>
        <h1 class="visual-title">Alege o parolă nouă și continuă <em>expediția.</em></h1>
        <p class="visual-copy">Linkul de resetare a fost verificat. Creează o parolă puternică, diferită de cea folosită anterior.</p>
        <div class="trail-note"><span class="trail-note-icon"><svg width="23" height="23" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="10" width="16" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg></span><span><strong>Sesiune securizată</strong><span>Linkul este valid pentru o singură resetare și expiră automat după salvare.</span></span></div>
      </div>
    </section>
    ...
    <section class="auth">
      <div class="auth-inner">
        ...
        <article class="form-card">
          <div id="formState">
            <p class="form-kicker">Resetare parolă</p>
            <h2 class="form-title">Setează parola nouă.</h2>
            <p class="form-copy">Folosește minimum 8 caractere și combină litere mari, litere mici și cifre.</p>
            <div class="token-status"><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s7-4.4 7-11V5l-7-2-7 2v5c0 6.6 7 11 7 11Z"/><path d="m9 12 2 2 4-5"/></svg><span><strong>Link verificat</strong><span>Cont identificat: a••••i@email.ro · link valabil încă 18 minute</span></span></div>
            <form class="form" id="passwordForm" novalidate>
              <div class="field">
                <label for="password">Parolă nouă</label>
                <div class="input-wrap"><input class="input password" id="password" name="password" type="password" autocomplete="new-password" placeholder="Minimum 8 caractere" required><button class="toggle-pass" type="button" data-target="password" aria-label="Afișează parola"><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg></button></div>
                <div class="strength" id="strength" data-level="0"><div class="strength-bars"><span></span><span></span><span></span><span></span></div><div class="strength-label"><span id="strengthText">Introdu parola</span><span id="strengthHint">0/4</span></div></div>
                <div class="rules" id="rules"><div class="rule" data-rule="length"><i>•</i>Minimum 8 caractere</div><div class="rule" data-rule="upper"><i>•</i>O literă mare</div><div class="rule" data-rule="lower"><i>•</i>O literă mică</div><div class="rule" data-rule="number"><i>•</i>O cifră</div></div>
                <div class="field-error" id="passwordError"></div>
              </div>
              <div class="field">
                <label for="confirmPassword">Confirmă parola</label>
                <div class="input-wrap"><input class="input password" id="confirmPassword" name="confirmPassword" type="password" autocomplete="new-password" placeholder="Repetă parola" required><button class="toggle-pass" type="button" data-target="confirmPassword" aria-label="Afișează parola"><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg></button></div>
                <div class="field-error" id="confirmError"></div>
              </div>
              <button class="primary" id="submitButton" type="submit"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 15v2m-6 4h12a2 2 0 0 0 2-2v-7a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2Z"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg><span>Salvează parola</span></button>
            </form>
            <div class="security"><div class="security-item"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s7-4.4 7-11V5l-7-2-7 2v5c0 6.6 7 11 7 11Z"/></svg>Criptare securizată</div><div class="security-item"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-3-6.7"/><path d="M21 3v6h-6"/></svg>Deconectare automată</div></div>
          </div>
          <div id="successState" hidden>
            <p class="form-kicker">Parolă actualizată</p>
            <h2 class="form-title">Contul este din nou al tău.</h2>
            <div class="success"><div class="success-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s7-4.4 7-11V5l-7-2-7 2v5c0 6.6 7 11 7 11Z"/><path d="m9 12 2 2 4-5"/></svg></div><h3>Parola a fost schimbată.</h3><p>Toate celelalte sesiuni active au fost închise pentru protecția contului.</p><span class="success-badge">Securitate actualizată</span><div class="success-actions"><a class="primary" href="/autentificare">Intră în cont</a><a class="secondary" href="/">Înapoi la homepage</a></div></div>
          </div>
        </article>
      </div>
    </section>
...
(() => {
  ...
  $('#menuOpen')?.addEventListener('click',openMenu); $('#menuClose')?.addEventListener('click',closeMenu); document.addEventListener('keydown',e=>{if(e.key==='Escape')closeMenu();});

  const form=$('#passwordForm'), pass=$('#password'), confirm=$('#confirmPassword'), strength=$('#strength'), strengthText=$('#strengthText'), strengthHint=$('#strengthHint'), passErr=$('#passwordError'), confirmErr=$('#confirmError'), submit=$('#submitButton');
  const checks=v=>({length:v.length>=8,upper:/[A-Z]/.test(v),lower:/[a-z]/.test(v),number:/\d/.test(v)});
  const updateStrength=()=>{
    const c=checks(pass.value), score=Object.values(c).filter(Boolean).length;
    strength.dataset.level=String(score);
    const labels=['Introdu parola','Slabă','Acceptabilă','Bună','Puternică']; strengthText.textContent=labels[score]; strengthHint.textContent=`${score}/4`;
    document.querySelectorAll('.rule').forEach(el=>{const ok=c[el.dataset.rule];el.classList.toggle('ok',ok);el.querySelector('i').textContent=ok?'✓':'•';});
    if(pass.classList.contains('error')) validate();
  };
  const validate=()=>{
    const c=checks(pass.value), strong=Object.values(c).every(Boolean), same=confirm.value && confirm.value===pass.value;
    pass.classList.toggle('error',!strong); confirm.classList.toggle('error',!same);
    passErr.textContent=strong?'':'Parola trebuie să respecte toate cele patru reguli.';
    confirmErr.textContent=same?'':'Parolele nu coincid.';
    return strong&&same;
  };
  pass.addEventListener('input',updateStrength); confirm.addEventListener('input',()=>{if(confirm.classList.contains('error'))validate();});
  document.querySelectorAll('.toggle-pass').forEach(btn=>btn.addEventListener('click',()=>{const input=document.getElementById(btn.dataset.target);const show=input.type==='password';input.type=show?'text':'password';btn.setAttribute('aria-label',show?'Ascunde parola':'Afișează parola');}));
  form.addEventListener('submit',e=>{e.preventDefault();if(!validate()){(pass.classList.contains('error')?pass:confirm).focus();return;}submit.disabled=true;submit.querySelector('span').textContent='Se salvează…';const qp=new URLSearchParams(location.search);(async()=>{try{const r=await fetch('/api/proxy.php?action=reset-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:qp.get('token')||'',email:qp.get('email')||'',password:pass.value,password_confirmation:confirm.value})});const d=await r.json().catch(()=>({}));if(r.ok&&d.success){$('#formState').hidden=true;$('#successState').hidden=false;$('#successState').animate([{opacity:0,transform:'translateY(14px)'},{opacity:1,transform:'translateY(0)'}],{duration:420,easing:'cubic-bezier(.22,.8,.2,1)',fill:'both'});}else{submit.disabled=false;submit.querySelector('span').textContent='Salvează parola';const t=$('#toast');const msg=(d&&(d.message||(d.errors&&d.errors.email&&d.errors.email[0])))||'Linkul de resetare este invalid sau a expirat.';if(t){t.textContent=msg;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2600);}}}catch(err){submit.disabled=false;submit.querySelector('span').textContent='Salvează parola';}})();});
  if(!matchMedia('(prefers-reduced-motion: reduce)').matches) document.querySelectorAll('.visual-content > *').forEach((el,i)=>el.animate([{opacity:0,transform:'translateY(20px)'},{opacity:1,transform:'translateY(0)'}],{duration:650,delay:120+i*90,easing:'cubic-bezier(.22,.8,.2,1)',fill:'both'}));
})();
</script>
</body>
</html>