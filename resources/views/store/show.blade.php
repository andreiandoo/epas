...
<body class="min-h-screen bg-gray-50 text-gray-800 antialiased" x-data="{ cartCount: {{ $cartCount }}, inCart: {{ $inCart ? 'true' : 'false' }} }">
    ...
    <main class="py-12">
        <div class="max-w-7xl mx-auto px-4">
            ...
            <!-- Related Microservices - Continuous Scrolling Marquee -->
            @if($relatedMicroservices->isNotEmpty())
                <div class="mt-16">
                    <h2 class="text-2xl font-bold text-gray-900 mb-6 text-center">Related Microservices</h2>
                    <div class="relative overflow-hidden">
                        <div class="marquee-container">
                            <div class="marquee-content flex gap-6 animate-marquee">
                                @foreach($relatedMicroservices as $related)
                                    <a href="{{ route('store.show', $related->slug) }}" class="flex-shrink-0 w-72 bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                                        <div class="flex items-center gap-3 mb-3">
                                            @if($related->icon_image)
                                                <img src="{{ Storage::url($related->icon_image) }}" alt="" class="w-10 h-10 rounded-lg">
                                            @else
