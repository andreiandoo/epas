...
<div class="db" x-data="{ tab: 'overview' }">
    ...
    <div x-show="tab === 'geographic'" x-cloak>
        ...
        <div class="g3" style="margin-top:14px;">
            ...
            @if(count($topCities))<div class="card"><div class="card-h">Top Cities</div><div class="card-b"><table class="tbl"><thead><tr><th>#</th><th>City</th><th>Tickets</th><th>Unique Buyers</th></tr></thead><tbody>@foreach($topCities as $i=>$r)<tr><td>{{ $i+1 }}</td><td>{{ $r->name }}</td><td><span class="badge">{{ number_format($r->tickets_count) }}</span></td><td>{{ number_format($r->fans_count) }}</td></tr>@endforeach</tbody></table></div></div>@endif
            @if(count($topCounties))<div class="card"><div class="card-h">Top Counties</div><div class="card-b"><table class="tbl"><thead><tr><th>#</th><th>County</th><th>Tickets</th><th>Unique Buyers</th></tr></thead><tbody>@foreach($topCounties as $i=>$r)<tr><td>{{ $i+1 }}</td><td>{{ $r->name }}</td><td><span class="badge">{{ number_format($r->tickets_count) }}</span></td><td>{{ number_format($r->fans_count) }}</td></tr>@endforeach</tbody></table></div></div>@endif
        </div>
        @endif
