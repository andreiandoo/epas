<x-filament-panels::page>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {{-- Total Revenue --}}
        <div class="fi-section rounded-xl bg-white/50 dark:bg-white/5 backdrop-blur-sm p-6 shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10">
        ...
        <div class="fi-section rounded-xl bg-white/50 dark:bg-white/5 backdrop-blur-sm p-6 shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    @if($totalCapacity > 0)
                        <div class="mt-1 h-2 w-full bg-gray-300 dark:bg-gray-700 rounded-full">
        ...
        <div class="fi-section rounded-xl bg-white/50 dark:bg-white/5 backdrop-blur-sm p-6 shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10">
            <div class="flex items-center gap-4">
                <div class="rounded-lg bg-info-500/10 p-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="lg:col-span-2">
            <div class="fi-section rounded-xl bg-white/50 dark:bg-white/5 backdrop-blur-sm shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10">
                <div class="fi-section-header flex items-center gap-3 px-6 py-4 border-b border-gray-200/50 dark:border-white/10">
                    <x-heroicon-o-ticket class="h-5 w-5 text-gray-500 dark:text-gray-400" />
                    <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Ticket Types</h3>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead class="bg-gray-100/50 dark:bg-white/5">
                            <tr>
                                <th class="px-6 py-3 text-left font-medium text-gray-600 dark:text-gray-400">Name</th>
                                <th class="px-6 py-3 text-right font-medium text-gray-600 dark:text-gray-400">Price</th>
                                <th class="px-6 py-3 text-right font-medium text-gray-600 dark:text-gray-400">Sold</th>
                                <th class="px-6 py-3 text-right font-medium text-gray-600 dark:text-gray-400">Available</th>
                                <th class="px-6 py-3 text-right font-medium text-gray-600 dark:text-gray-400">Revenue</th>
                                <th class="px-6 py-3 text-center font-medium text-gray-600 dark:text-gray-400">Progress</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200/50 dark:divide-white/10">
                            @forelse($ticketTypes as $type)
                                <tr class="hover:bg-gray-100/30 dark:hover:bg-white/5">
                                    <td class="px-6 py-4 font-medium text-gray-900 dark:text-gray-100">
                                        {{ $type['name'] }}
                                        @if($type['status'] !== 'active')
                                            <span class="ml-2 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium
                                                {{ $type['status'] === 'hidden' ? 'bg-gray-200/50 text-gray-600 dark:bg-gray-700/50 dark:text-gray-400' : 'bg-red-100/50 text-red-600 dark:bg-red-900/30 dark:text-red-400' }}">
                                                {{ ucfirst($type['status']) }}
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            <div class="w-20 h-2 bg-gray-300 dark:bg-gray-700 rounded-full">
                                            ...
                                            <span class="text-xs text-gray-600 dark:text-gray-400 w-10">{{ $type['percentage'] }}%</span>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                        ...
                        @if(count($ticketTypes) > 0)
                            <tfoot class="bg-gray-100/50 dark:bg-white/5 font-semibold">
                                <tr>
                                    <td class="px-6 py-3 text-gray-900 dark:text-gray-100">Total</td>
                                    <td class="px-6 py-3"></td>
                                    <td class="px-6 py-3 text-right text-gray-900 dark:text-gray-100">{{ number_format($totalSold) }}</td>
        ...
        <div>
            <div class="fi-section rounded-xl bg-white/50 dark:bg-white/5 backdrop-blur-sm shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10">
                <div class="fi-section-header flex items-center gap-3 px-6 py-4 border-b border-gray-200/50 dark:border-white/10">
                    <x-heroicon-o-shopping-cart class="h-5 w-5 text-gray-500 dark:text-gray-400" />
                    <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Order Status</h3>
                </div>
                <div class="p-6 space-y-4">
                    ...
                    @foreach($statusData as $status)
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-gray-100">{{ $status['value'] }}</span>
                        </div>
                    @endforeach
                    <div class="pt-4 border-t border-gray-200/50 dark:border-white/10">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-bold text-lg text-gray-900 dark:text-gray-100">{{ $orderStats['total'] }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    {{-- Sales Chart --}}
    <div class="fi-section rounded-xl bg-white/50 dark:bg-white/5 backdrop-blur-sm shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10 mb-6">
        <div class="fi-section-header flex items-center gap-3 px-6 py-4 border-b border-gray-200/50 dark:border-white/10">
            <x-heroicon-o-chart-bar class="h-5 w-5 text-gray-500 dark:text-gray-400" />
            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Daily Sales (Last 30 Days)</h3>
        </div>
        <div class="p-6">
    ...
    @if($analytics['available'] ?? false)
        <div class="fi-section rounded-xl bg-white/50 dark:bg-white/5 backdrop-blur-sm shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10">
            <div class="fi-section-header flex items-center gap-3 px-6 py-4 border-b border-gray-200/50 dark:border-white/10">
                <x-heroicon-o-eye class="h-5 w-5 text-gray-500 dark:text-gray-400" />
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Page Analytics</h3>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-gray-400">Total Page Views</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ number_format($analytics['total_views'] ?? 0) }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600 dark:text-gray-400">Unique Visitors</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ number_format($analytics['unique_sessions'] ?? 0) }}</p>
                    </div>
                    @if(!empty($analytics['top_referrers']))
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">Top Referrers</p>
                            <ul class="space-y-1">
                    ...
                    @if(!empty($analytics['top_countries']))
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">Top Countries</p>
                            <ul class="space-y-1">
