...
if (!empty($hub['filter'])) {
    $v2Scripts = array_merge($v2Scripts ?? [], ['hub-filter.js']);
}
// The short hero shows no picture, so there is nothing to preload.
$v2HeadExtra = !$hubTight && !empty($hub['image']) ? '<link rel="preload" as="image" href="' . v2_e($hub['image']) . '" fetchpriority="high">' : '';
...
<main id="main" tabindex="-1">
  ...
  <section class="kres hub-res" aria-labelledby="hub-res-h">
    <div class="wrap">
      ...
      <?php if (!empty($hub['filter'])): ?>
      <?php include __DIR__ . '/hub-filter.php'; ?>
      <?php elseif (!empty($hub['filters'])): ?>
      ...
      <ul class="xp-grid" data-reveal>
        ...
        <li class="xp">
          <a href="<?= v2_e($it['href']) ?>"<?= !empty($it['aria']) ? ' aria-label="' . v2_e($it['aria']) . '"' : '' ?>>
