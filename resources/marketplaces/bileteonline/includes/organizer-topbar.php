...
<div class="py-1 text-xs font-bold tracking-wide text-center text-black bg-amber-500">
    STAGE API â€” Datele sunt de test
        ...
        <div class="flex items-center gap-2">
            ...
            <div class="relative dropdown">
                ...
                <div class="absolute right-0 z-50 w-56 py-2 mt-2 overflow-hidden bg-white border shadow-lg dropdown-menu top-full rounded-xl border-border">
                    ...
                    <button onclick="BileteOnlineAuth.logoutOrganizer()" class="flex items-center w-full gap-2 px-4 py-2 text-sm text-error hover:bg-surface">
...
(function() {
    try {
        const orgData = JSON.parse(localStorage.getItem('bileteonline_organizer_data') || 'null');
        if (orgData) {
...
(function() {
    ...
    window.addEventListener('load', function() {
        ...
        async function loadSearchData() {
            ...
            try {
                if (typeof BileteOnlineAPI === 'undefined') return [];
        ...
        async function performSearch(query) {
            ...
            searchResults.innerHTML = results.map(event => {
                ...
                const statusLabels = { published: 'Publicat', draft: 'CiornÄƒ', ended: 'ÃŽncheiat', pending_review: 'ÃŽn aÈ™teptare', cancelled: 'Anulat' };
                ...
                const subLine = [dateStr, locationStr].filter(Boolean).join(' Â· ');
                return `
