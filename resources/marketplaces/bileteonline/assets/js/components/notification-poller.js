 ...
 */
const BileteOnlineNotificationPoller = (function() {
    let pollingInterval = null;
    let lastNotificationCount = 0;
    let lastNotificationId = null;
    let isInitialized = false;
    const POLL_INTERVAL = 30000; // 30 seconds
    const STORAGE_KEY_LAST_ID = 'bileteonline_last_notification_id';
    ...
    async function fetchNotifications() {
        try {
            ...
            const response = await BileteOnlineAPI.get('/organizer/notifications?per_page=5&read=0');
    ...
    function updateUI(data) {
        ...
        if (listEl && notifications.length > 0) {
            listEl.innerHTML = notifications.slice(0, 5).map(n => {
                ...
                return `
                    <a href="${n.action_url || '/organizator/notifications'}" onclick="BileteOnlineNotificationPoller.markAsRead(${n.id})" class="flex items-start gap-3 p-4 hover:bg-surface border-b border-border last:border-0 transition-colors ${n.is_read ? 'opacity-60' : ''}">
    ...
    async function checkNotifications() {
        ...
        if (notifications.length > 0) {
            ...
            if (storedLastId && newestId > parseInt(storedLastId)) {
                ...
                if (typeof BileteOnlineNotificationSound !== 'undefined') {
                    BileteOnlineNotificationSound.play(soundType);
                }

                // Show toast notification for the newest one
                if (typeof BileteOnlineNotifications !== 'undefined') {
                    BileteOnlineNotifications.show(newestNotification.title, newestNotification.color || 'info');
                }
            }
    ...
    function markAsRead(notificationId) {
        if (!notificationId || typeof BileteOnlineAPI === 'undefined') return;
        try {
            BileteOnlineAPI.post('/organizer/notifications/' + notificationId + '/read', {});
        } catch (e) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const orgData = localStorage.getItem('bileteonline_organizer_data');
    if (orgData) {
        // Start polling after a short delay to let other components initialize
        setTimeout(function() {
            BileteOnlineNotificationPoller.start();
        }, 1000);
    }
});
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = BileteOnlineNotificationPoller;
}
