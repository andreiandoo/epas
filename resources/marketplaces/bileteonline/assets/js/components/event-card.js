...
const BileteOnlineEventCard = {
    ...
    render(eventData, options = {}) {
        ...
        if (event.isCancelled) {
            ...
            statusBadge = '<span class="absolute px-2 py-1 text-sm font-bold text-white uppercase rounded-lg top-3 right-3 bg-orange-500">AMÃ‚NAT</span>';
            ...
            if (event.isCancelled) {
                ...
                priceRowHtml = '<div class="px-3 flex items-center justify-between pt-1 border-t border-border">' +
                    ...
                    '<span class="text-xs ' + (event.isPostponed ? 'text-orange-600 font-semibold' : event.isSoldOut ? 'text-gray-600 font-semibold' : 'text-muted') + '">' +
                        (event.isPostponed ? 'AmÃ¢nat' : event.isSoldOut ? 'Sold Out' : '') +
                    '</span>' +
                    (showCategory && event.categoryName ?
                        '<span class="mobile:hidden cat-pill font-semibold text-white uppercase rounded-lg bg-black/60 backdrop-blur-sm">' + this.escapeHtml(event.categoryName) + '</span>' :
                        '') +
    ...
    renderPromoted(eventData, options = {}) {
        ...
        if (event.isCancelled) {
            ...
            statusBadge = '<span class="absolute px-2 py-1 text-xs font-bold text-white uppercase rounded-lg top-3 right-3 z-10 bg-orange-500">AMÃ‚NAT</span>';
        } else if (event.isSoldOut) {
    ...
    renderHorizontal(eventData, options = {}) {
        ...
        if (showPrice) {
            if (event.isSoldOut) {
                ...
                buttonHtml = showBuyButton ? '<button class="py-2.5 px-5 bg-secondary hover:bg-secondary/90 rounded-lg text-white text-sm font-semibold transition-all">CumpÄƒrÄƒ bilete</button>' : '';
            }
        }
     ...
     * Uses BileteOnlineDataTransformer if available, otherwise basic normalization
     *
    ...
    normalizeEvent(apiEvent) {
        ...
        if (typeof BileteOnlineDataTransformer !== 'undefined') {
            return BileteOnlineDataTransformer.normalizeEvent(apiEvent);
        }
        ...
        return {
            ...
            minPrice: minPrice,
            isPasswordProtected: apiEvent.is_password_protected || false,
            priceFormatted: (apiEvent.is_password_protected)
                ? '<span class="flex items-center gap-1"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>Eveniment Privat</span>'
                : minPrice > 0 ? (hasMultipleTicketTypes ? '<span class="font-semibold text-slate-700 text-xs">De la</span> ' : '<span class="font-semibold text-slate-800 text-xs">Bilete: </span>') + minPrice + ' lei' : 'Gratuit',
...
window.BileteOnlineEventCard = BileteOnlineEventCard;
