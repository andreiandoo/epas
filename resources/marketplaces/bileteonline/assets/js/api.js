...
const BileteOnlineAPI = {
    ...
    getProxyAction(endpoint, method) {
        ...
        if (endpoint === '/organizer/contract/sign') return 'organizer.contract.sign';
        ...
        if (endpoint.match(/\/organizer\/events\/\d+\/leisure\/scans-detail$/)) return 'organizer.event.leisure.scans-detail';
        if (endpoint.match(/\/organizer\/events\/\d+\/leisure\/scans$/)) return 'organizer.event.leisure.scans';
    ...
    getProxyParams(endpoint) {
        ...
        const venueOrderMatch = endpoint.match(/^\/organizer\/events\/(\d+)\/leisure\/orders\/(\d+)/);
        if (venueOrderMatch) {
            return `event=${encodeURIComponent(venueOrderMatch[1])}&order_id=${encodeURIComponent(venueOrderMatch[2])}`;
        }
