...
const SearchPage = {
    ...
    async performSearch(query) {
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                // CAPI Search event â€” fires when results land. Backend
    ...
    updateSummaryBadges(eventsCount, artistsCount, locationsCount) {
        ...
        if (artistsBadge) {
            if (artistsCount > 0) {
                artistsBadge.textContent = `${artistsCount} ${artistsCount === 1 ? 'artist' : 'artiÈ™ti'}`;
        ...
        if (locationsBadge) {
            if (locationsCount > 0) {
                locationsBadge.textContent = `${locationsCount} ${locationsCount === 1 ? 'locaÈ›ie' : 'locaÈ›ii'}`;
    ...
    renderEventsSection(events) {
        ...
        if (grid) {
            // Use BileteOnlineEventCard for consistent rendering across the site
            if (typeof BileteOnlineEventCard !== 'undefined') {
                grid.innerHTML = BileteOnlineEventCard.renderMany(events, {
                    urlPrefix: '/bilete/',
                    showCategory: true,
                    showPrice: true,
                    showVenue: true
