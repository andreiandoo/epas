...
const CityPage = {
    ...
    async loadCityData() {
        try {
            const response = await BileteOnlineAPI.get('/locations/cities/' + encodeURIComponent(this.city));
    ...
    updatePageWithCityData(city) {
        // Update page title
        document.title = 'Evenimente Ã®n ' + city.name + ' - AmBilet.ro';
        ...
        if (descEl) {
            ...
            if (regionName) {
                descEl.textContent = 'DescoperÄƒ cele mai bune evenimente din ' + city.name + ', ' + regionName + '.';
        ...
        try {
            ...
            if (response.data && response.data.length > 0) {
                ...
                if (response.meta) {
                    ...
                    const pagination = BileteOnlinePagination.normalize(response.meta);
