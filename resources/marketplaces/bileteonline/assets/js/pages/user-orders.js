...
const UserOrders = {
    ...
    async loadOrders() {
        try {
            const response = await BileteOnlineAPI.customer.getOrders({ per_page: 50 });
    ...
    renderOrderCard(order) {
        ...
        const statusLabel = {
            'confirmed': 'CONFIRMATÄ‚',
            'completed': 'FINALIZATÄ‚',
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="mobile:flex-col mobile:flex">
                            <div class="flex-1 min-w-0 mobile:mb-1">
                                ...
                                <p class="text-sm text-muted">${this.formatDateTime(order.created_at)} â€¢ ${ticketCount < 2 ? '1 bilet' : ticketCount + ' bilete'} ${ticketName}</p>
                            </div>
                            <div class="mobile:items-center mobile:gap-4 mobile:flex">
                                <div class="flex items-center flex-shrink-0 text-right gap-x-4">
                                    ${order.status === 'refunded' ?
                                        `<p class="font-bold line-through text-muted">${paidTotal.toFixed(2)} lei</p><p class="text-xs text-error">RambursatÄƒ</p>` :
    ...
    renderOrderDetails(order) {
        ...
        return `
            ...
            <div class="mb-6">
                <h4 class="mb-3 text-sm font-semibold text-secondary">Status comandÄƒ</h4>
                ...
                <div class="flex justify-between mt-2 text-xs text-muted">
                    <span>Comanda plasatÄƒ</span>
                    <span>Plata confirmatÄƒ</span>
            ...
            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                <div>
                    ...
                    <div class="space-y-2">
                        ...
                        ${insurance > 0 ? `
                        <div class="flex justify-between text-sm">
                            <span class="text-muted">TaxÄƒ rambursare bilete.online</span>
                            <span class="text-secondary">${insurance.toFixed(2)} lei</span>
                        </div>
                        ` : ''}
                        ${order.discount ? `
                        <div class="flex justify-between text-sm">
                            <span class="text-muted">Cod promoÈ›ional ${order.promo_code ? `(${order.promo_code})` : ''}</span>
                            <span class="text-success">-${parseFloat(order.discount).toFixed(2)} lei</span>
                        </div>
                        ` : ''}
                        <hr class="border-border">
                        <div class="flex justify-between font-semibold">
                            <span class="text-secondary">Total plÄƒtit</span>
                ...
                <div>
                    <h4 class="mb-2 text-sm font-semibold text-secondary">InformaÈ›ii platÄƒ</h4>
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="${(order.payment_status === 'paid' || order.status === 'paid' || order.status === 'confirmed') ? 'text-success' : 'text-warning'}">${(order.payment_status === 'paid' || order.status === 'paid' || order.status === 'confirmed') ? 'PlÄƒtit' : order.payment_status === 'pending' ? 'ÃŽn aÈ™teptare' : (order.payment_status || order.status)}</span>
                        </div>
                        ${order.paid_at ? `
                        <div class="flex justify-between">
                            <span class="text-muted">Data plÄƒÈ›ii</span>
        ...
        return `
            <div class="p-4 mb-4 border bg-error/5 border-error/20 rounded-xl">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="font-semibold text-error">${order.refund_reason || 'ComandÄƒ rambursatÄƒ'}</p>
                        <p class="mt-1 text-sm text-muted">Rambursarea a fost procesatÄƒ automat Ã®n contul tÄƒu pe ${order.refund_date}.</p>
                    </div>
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            <span class="text-muted">Suma rambursatÄƒ</span>
                            <span class="font-medium text-success">${order.refunded_amount} lei</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-muted">Data rambursÄƒrii</span>
