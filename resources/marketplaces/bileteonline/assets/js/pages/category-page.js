...
const CategoryPage = {
    ...
    async init(categorySlug) {
        ...
        // General featured events are rendered by FeaturedCarousel (see category.php);
        // loadCategoryFeaturedEvents() below still handles the premium "is_category_featured"
    ...
    async loadCategoryInfo() {
        ...
        try {
            const response = await BileteOnlineAPI.get('event-categories');
    ...
    async loadGenres() {
        ...
        try {
            const response = await BileteOnlineAPI.get('/event-genres?category=' + this.category);
    ...
    async loadCities() {
        try {
            ...
            const response = await BileteOnlineAPI.get('/events/cities' + params);
    ...
    async loadCategoryFeaturedEvents() {
        ...
        try {
            ...
            const response = await BileteOnlineAPI.get('/events/featured?' + params.toString());
            ...
            if (response.data) {
                ...
                if (events.length > 0) {
                    ...
                    if (sortValue.startsWith('date_')) {
                        ...
                        // Flat grid for price/popularity sorts â€” preserve API order
                        container.innerHTML = events.map(e => BileteOnlineEventCard.render(e)).join('');
                ...
                // Render pagination
                const pagination = BileteOnlinePagination.normalize(meta);
                BileteOnlinePagination.render({
    ...
    renderMonthGroup(group) {
        const eventsHtml = group.events.map(e => BileteOnlineEventCard.render(e)).join('');
    ...
    goToPage(page) {
        ...
        this.loadEvents();
        BileteOnlinePagination.scrollTo(300);
    },
