document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        const [profileRes, dataRes, badgesRes] = await Promise.all([
            BileteOnlineAPI.customer.getProfile(),
            BileteOnlineAPI.customer.getProfileData(),
            BileteOnlineAPI.customer.getBadges().catch(() => ({ success: false }))
        ]);
    ...
    const customerTypeEmoji = profileData.customer_type_emoji || 'ðŸŒŸ';
    ...
    if (typeDesc) {
        const typeDescriptions = {
            ...
            'Sportiv': 'EÈ™ti fan al sportului! Evenimentele sportive dominÄƒ lista ta.',
            'Cultural': 'Cultura È™i arta te definesc! Teatrul È™i evenimentele culturale sunt preferatele tale.',
            'Festivalier': 'Festivalurile sunt locul tÄƒu preferat! Energie È™i distracÈ›ie la maximum.',
            'Comedian Fan': 'RÃ¢sul e cea mai bunÄƒ medicinÄƒ! Stand-up comedy-ul e genul tÄƒu.',
            'Explorator': 'EÈ™ti curios È™i deschis! Explorezi diverse tipuri de evenimente.',
        };
...
function toggleEmptyState(containerId, isEmpty) {
    ...
    if (!container) return;
    if (isEmpty) {
        container.innerHTML = '<p class="py-4 text-sm text-center text-muted">Nu sunt suficiente date Ã®ncÄƒ. ParticipÄƒ la mai multe evenimente!</p>';
    }
}
        ...
        <div>
            <div class="flex items-center justify-between mb-2 text-sm">
                <span class="flex items-center gap-2 font-medium text-secondary">
                    <span class="text-lg">${genre.emoji || 'ðŸŽµ'}</span> ${genre.name}
                </span>
...
function renderBadges(badges) {
    ...
    if (!badges.length) {
        container.innerHTML = '<p class="py-4 text-sm text-center text-muted col-span-full">Niciun badge cÃ¢È™tigat Ã®ncÄƒ. ContinuÄƒ sÄƒ participi la evenimente!</p>';
        return;
    }
    ...
    const badgesHtml = badges.slice(0, 5).map((badge, i) => {
        const emoji = badge.icon ? `<img src="${badge.icon}" class="w-8 h-8" alt="${badge.name}">` : 'ðŸ†';
        return `
