const ThankYouPage = {
    ...
    fireTrackPurchase() {
        try {
            ...
            // core_customers.email / last_fbclid for this visitor â€” the
            // single missing link that breaks ROAS attribution on
            // bilete.online (visitors get a CoreCustomer row by visitor_id
    ...
    renderFailedPayment() {
        ...
        const heading = document.querySelector('h1');
        if (heading) heading.textContent = 'Plata nu a fost procesatÄƒ';

        const printingText = document.getElementById('printingText');
        if (printingText) {
            printingText.textContent = 'Din pÄƒcate, plata nu a putut fi finalizatÄƒ. Te rugÄƒm sÄƒ verifici datele cardului È™i sÄƒ Ã®ncerci din nou.';
            printingText.classList.remove('text-lg');
            printingText.classList.add('text-base');
        }
        ...
        if (statusBadge) {
            ...
            statusBadge.textContent = 'EÈ™uatÄƒ';
        }
    ...
    renderPendingPayment() {
        ...
        const heading = document.querySelector('h1');
        if (heading) heading.textContent = 'Plata este Ã®n procesare';

        const printingText = document.getElementById('printingText');
        if (printingText) printingText.textContent = 'Plata ta este Ã®n curs de verificare. Vei primi biletele pe email imediat ce plata este confirmatÄƒ.';
        ...
        if (emailSection) {
            ...
            if (emailText) emailText.textContent = 'Vei primi biletele pe email dupÄƒ confirmarea plÄƒÈ›ii';
        ...
        if (statusBadge) {
            ...
            statusBadge.textContent = 'ÃŽn aÈ™teptare';
        }
        ...
        if (event && eventInfo) {
            ...
            eventInfo.innerHTML = `
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-muted">${eventDate}${eventTime ? ' â€¢ ' + eventTime : ''}</p>
            ...
            html += order.items.map(item => `
                <div class="flex items-center justify-between py-2 border-b border-border last:border-0">
                    <div>
                        ...
                        <span class="ml-1 text-sm text-muted">Ã— ${item.quantity}</span>
                    </div>
                    <span class="font-semibold">${BileteOnlineUtils.formatCurrency(item.total)}</span>
                </div>
            `).join('');
        ...
        document.getElementById('paymentSummary').innerHTML = `
            ...
            <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <span>${BileteOnlineUtils.formatCurrency(insuranceAmount)}</span>
                </div>` : ''}
                ${discount > 0 ? `
                <div class="flex justify-between text-success">
                    <span>Reducere</span>
                    <span>-${BileteOnlineUtils.formatCurrency(discount)}</span>
                </div>` : ''}
                <div class="flex justify-between pt-2 text-lg font-bold border-t border-border">
                    <span>Total</span>
                    <span class="text-primary">${BileteOnlineUtils.formatCurrency(total)}</span>
                </div>
            </div>
        `;
    ...
    renderOrderData() {
        ...
        document.getElementById('buyerEmail').textContent = order.customer_email || 'Email-ul tÄƒu';
        ...
        if (order.items && order.items.length > 0) {
            let html = `<h4 class="mb-3 font-semibold text-secondary">Bilete achiziÈ›ionate</h4>`;
            html += order.items.map(item => `
                <div class="flex items-center justify-between py-2 border-b border-border last:border-0">
                    <div>
                        <span class="font-medium text-secondary">${item.name}</span>
                    ...
                    ${seatedTickets.map(t => `
                        <div class="flex items-center gap-2 py-1 text-sm">
                            ...
                            <span class="font-medium text-secondary">
                                ${[t.seat.section_name, t.seat.row_label ? 'RÃ¢nd ' + t.seat.row_label : '', t.seat.seat_number ? 'Loc ' + t.seat.seat_number : ''].filter(Boolean).join(', ')}
                            </span>
                        </div>
                    `).join('')}
                </div>`;
    ...
    downloadTicketsLegacyPrint() {
        ...
        const ticketsHtml = order.tickets.map((ticket, idx) => {
            const seatInfo = ticket.seat ? [
                ...
                ticket.seat.row_label ? 'RÃ¢nd ' + ticket.seat.row_label : '',
            ...
            return `
                <div style="page-break-inside: avoid; border: 2px solid #1E293B; border-radius: 12px; padding: 24px; margin-bottom: 24px; max-width: 500px;">
                    ...
                    <div style="display: flex; gap: 24px; margin-bottom: 12px;">
                        ...
                        <div><div style="font-size: 11px; color: #64748B;">LOCAÈšIE</div><div style="font-weight: 600;">${venue}${event?.city ? ', ' + event.city : ''}</div></div>
                    </div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                        ...
                        <div style="text-align: right;"><div style="font-size: 11px; color: #64748B;">PREÈš</div><div style="font-weight: 700; color: #A51C30;">${BileteOnlineUtils.formatCurrency(ticket.price)}</div></div>
                    </div>
    ...
    addToCalendar() {
        ...
        if (!startDate) {
            if (typeof BileteOnlineNotifications !== 'undefined') {
                BileteOnlineNotifications.info('Data evenimentului nu este disponibilÄƒ.');
            }
            return;
        }
        ...
        const gcalUrl = 'https://www.google.com/calendar/render?action=TEMPLATE'
            ...
            + '&details=' + encodeURIComponent('Bilete achiziÈ›ionate pe ' + (window.BILETEONLINE?.siteName || 'bilete.online'));
    ...
    renderTickets(tickets) {
        ...
        if (total === 0) {
            document.getElementById('ticketsCount').textContent = 'Nu existÄƒ bilete';
            return;
        }
            ...
            <div class="ticket-card" data-index="${idx}">
                ...
                <div class="ticket-card-body">
                    ...
                    <div class="mt-4 space-y-3">
                        ...
                        <div>
                            <p class="text-xs tracking-wide uppercase text-muted">LocaÈ›ie</p>
                        ...
                        <div class="flex flex-wrap gap-4">
                            ${ticket.seat.section_name ? `
                            <div>
                                <p class="text-xs tracking-wide uppercase text-muted">SecÈ›iune</p>
                                <p class="font-semibold text-secondary">${ticket.seat.section_name}</p>
                            </div>` : ''}
                            ${ticket.seat.row_label ? `
                            <div>
                                <p class="text-xs tracking-wide uppercase text-muted">RÃ¢nd</p>
                        ...
                        <div class="flex items-center justify-between pt-2">
                            ...
                            <div class="text-right">
                                <p class="text-xs text-muted">PreÈ›</p>
                                <p class="font-bold text-primary">${BileteOnlineUtils.formatCurrency(ticket.price)}</p>
                            </div>
                        </div>
    ...
    copyLink() {
        ...
        navigator.clipboard.writeText(url).then(() => {
            if (typeof BileteOnlineNotifications !== 'undefined') {
                BileteOnlineNotifications.success('Link copiat Ã®n clipboard!');
            } else {
