...
const EventsPage = {
    ...
    groupEventsByMonth(events) {
        ...
        events.forEach(event => {
            const normalized = BileteOnlineDataTransformer.normalizeEvent(event);
     ...
     * Uses BileteOnlineEventCard.render() for grid view and BileteOnlineEventCard.renderHorizontal() for list view
     */
    renderMonthGroup(group) {
        ...
        // Use the unified BileteOnlineEventCard component based on current view
        let eventsHtml;
        if (this.view === 'list') {
            // List view - horizontal cards
            eventsHtml = group.events.map(item => BileteOnlineEventCard.renderHorizontal(item.raw, cardOptions)).join('');
        } else {
            // Grid view - standard grid cards
            eventsHtml = group.events.map(item => BileteOnlineEventCard.render(item.raw, cardOptions)).join('');
        }
    ...
    goToPage(page) {
        this.page = page;
        this.loadEvents();
        BileteOnlinePagination.scrollTo(400);
    },
    ...
    updateActiveFilters() {
        ...
        if (this.filters.venue) activeFilters.push({ key: 'venue', label: 'LocaÈ›ie: ' + this.filters.venue });
        ...
        tagsContainer.innerHTML = activeFilters.map(f =>
            '<span class="inline-flex items-center gap-1 px-3 py-1 text-sm font-medium text-gray-700 bg-gray-100 rounded-full">' +
                BileteOnlineEventCard.escapeHtml(f.label) +
                '<button onclick="EventsPage.removeFilter(\'' + f.key + '\')" aria-label="È˜terge filtrul" class="ml-1 text-gray-400 hover:text-gray-600" aria-label="È˜terge filtrul">' +
