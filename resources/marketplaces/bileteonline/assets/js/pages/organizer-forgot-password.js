const ForgotPage = {
    async submit(event) {
        ...
        try {
            const response = await BileteOnlineAPI.post('/organizer/forgot-password', { email });
            ...
            if (response.success) {
                ...
                if (typeof BileteOnlineNotifications !== 'undefined') {
                    BileteOnlineNotifications.error(response.message || 'A aparut o eroare. Incearca din nou.');
                }
                btn.disabled = false;
    ...
    async resend() {
        ...
        try {
            await BileteOnlineAPI.post('/organizer/forgot-password', { email });
        } catch (e) {}
