...
const HeroSlider = {
    ...
    async init() {
        try {
            const response = await BileteOnlineAPI.get('/events/featured?type=homepage&limit=12');
...
const CityEventsFilter = {
    ...
    async loadEvents() {
        try {
            const response = await BileteOnlineAPI.get('/marketplace-events?sort=latest&per_page=45');
            if (response.data) {
    ...
    renderEvents() {
        ...
        if (!events || events.length === 0) {
            container.innerHTML = '<p class="py-8 text-center text-muted col-span-full">Nu sunt evenimente disponibile pentru acest oraÈ™.</p>';
            return;
        }

        // Use BileteOnlineEventCard component for rendering
        if (typeof BileteOnlineEventCard !== 'undefined') {
            container.innerHTML = BileteOnlineEventCard.renderMany(events.slice(0, 45), {
                urlPrefix: '/bilete/',
                showCategory: true,
                showPrice: true,
                showVenue: true
...
const PromotedEvents = {
    async init() {
        try {
            ...
            const [promotedRes, recommendedRes] = await Promise.all([
                BileteOnlineAPI.get('/marketplace-events?promoted=1&limit=12').catch(() => ({ data: [] })),
                BileteOnlineAPI.get('/marketplace-events?recommended=1&limit=12').catch(() => ({ data: [] }))
            ]);
    ...
    render(events) {
        ...
        if (typeof BileteOnlineEventCard !== 'undefined' && typeof BileteOnlineEventCard.renderManyPromoted === 'function') {
            container.innerHTML = BileteOnlineEventCard.renderManyPromoted(events);
        } else {
            container.innerHTML = '<p class="text-muted col-span-full">Nu sunt evenimente disponibile.</p>';
        }
...
async function loadCategories() {
    try {
        const response = await BileteOnlineAPI.getCategories();
    ...
    container.innerHTML = categories.slice(0, 6).map(function(cat) {
        return '<a href="/' + cat.slug + '" class="flex items-center gap-3 px-3 py-2 bg-white border rounded-lg category-pill border-border group">' +
            '<div class="flex items-center justify-center w-8 h-8 transition-colors rounded-lg bg-primary/10 group-hover:bg-white/20">' +
                '<span class="text-lg">' + (icons[cat.slug] || 'ðŸŽ«') + '</span>' +
            '</div>' +
