 ...
 *
 * Dependencies: BileteOnlineAPI, BileteOnlineUtils, BileteOnlineAuth
 */
...
const VenuePage = {
    ...
    weekdayNames: ['Dum', 'Lun', 'Mar', 'Mie', 'Joi', 'Vin', 'SÃ¢m'],
    ...
    async init() {
        ...
        if (!this.venueSlug) {
            this.showError('LocaÈ›ia nu a fost gÄƒsitÄƒ');
            return;
        }
        ...
        if (typeof BileteOnlineAuth !== 'undefined' && BileteOnlineAuth.isOrganizer && BileteOnlineAuth.isOrganizer()) {
    ...
    async loadVenueData() {
        try {
            const response = await BileteOnlineAPI.getVenue(this.venueSlug);
            ...
            if (error.status === 404) {
                ...
                this.showError('Eroare la Ã®ncÄƒrcarea datelor');
            }
        }
    },
    ...
    transformApiData(apiData) {
        ...
        // Primary venue category (first one) â€” used as hero badge
        ...
        // Gallery â€” accept both facilities-style objects and plain strings
            ...
            hero.innerHTML =
                '<div class="flex items-center justify-center h-full bg-slate-100">' +
                    '<div class="text-center p-6">' +
                        ...
                        '<a href="/locatii" class="inline-block px-6 py-3 mt-4 text-white rounded-lg bg-primary hover:bg-primary-dark">Vezi toate locaÈ›iile</a>' +
                    '</div>' +
                '</div>';
        }
    },
    ...
    render() {
        ...
        // Page title
        document.title = venue.name + ' - ' + ((window.BILETEONLINE_CONFIG && window.BILETEONLINE_CONFIG.SITE_NAME) || 'AmBilet.ro');
    },

    /* â•â•â•â•â•â•â•â•â•â•â•â•â• HERO â•â•â•â•â•â•â•â•â•â•â•â•â• */
    renderHero(venue) {
        // Hero image â€” prefer portrait on narrow screens when provided
        ...
        // City is rendered separately in its own pill â€” keep this slot
        ...
        // Open-status pill â€” hide entirely when we have no schedule data to
        // avoid showing "ÃŽn funcÈ›ie de evenimente" without any basis for it.
    ...
    /* â•â•â•â•â•â•â•â•â•â•â•â•â• STATS TILES â•â•â•â•â•â•â•â•â•â•â•â•â• */
    renderStats(venue) {
        ...
        // Rating tile â€” gated on Google data
        ...
        // Year-built tile â€” only when we know the year
    ...
    /* â•â•â•â•â•â•â•â•â•â•â•â•â• ABOUT + AMENITIES â•â•â•â•â•â•â•â•â•â•â•â•â• */
    ...
    renderReviewsSection(gr) {
        ...
        if (subheader) {
            subheader.textContent = rating.toFixed(1) + ' din 5 Â· ' + this.formatNumber(reviewCount) + ' recenzii';
        }
        ...
        // Pick 3 reviews at random from those scoring â‰¥ 4.5 â€” done client-side
        ...
        //   â€¢ â‰¥ 4.5 stars â†’ preferred pool
        //   â€¢ if that pool has < 5 candidates, include 4+ to widen it
        //   â€¢ if we still have < 3, pad with remaining reviews by rating desc
        ...
        grid.innerHTML = displayReviews.map(function (r, i) {
            ...
            // so reviews read as "12 apr. 2026" instead of "Ã®n ultima sÄƒptÄƒmÃ¢nÄƒ".
            ...
            return '<div class="border border-slate-200 rounded-xl p-4">' +
                ...
                '</div>' +
                (text ? '<p class="text-sm text-slate-700 leading-relaxed">â€ž' + self.escapeHtml(text) + '"</p>' : '') +
            '</div>';
        }).join('');
    },
    ...
    renderEventFilterTabs(categories, totalEvents) {
        ...
        // category tabs even if they're all the same category â€” the design
    ...
    renderEvents(events, filterSlug) {
        ...
        if (filtered.length === 0) {
            container.innerHTML =
                '<div class="p-12 text-center">' +
                    ...
                    '</div>' +
                    '<h3 class="font-semibold text-slate-900 mb-1">' + (events && events.length ? 'Niciun eveniment Ã®n aceastÄƒ categorie' : 'Nu existÄƒ evenimente viitoare') + '</h3>' +
                    (events && events.length
                        ? '<p class="text-sm text-slate-500 mb-4">ÃŽncearcÄƒ o altÄƒ categorie.</p><button type="button" onclick="VenuePage.setEventFilter(\'all\')" class="text-sm font-semibold text-primary hover:text-primary-dark">â† Vezi toate evenimentele</button>'
                        : '<p class="text-sm text-slate-500">Revino mai tÃ¢rziu pentru evenimente noi.</p>') +
                '</div>';
        ...
        if (footer && footerCount) {
            const totalText = (events || []).length === filtered.length
                ? 'AfiÈ™ate ' + filtered.length + ' din ' + (events || []).length + ' evenimente'
                : 'AfiÈ™ate ' + filtered.length + ' din ' + (events || []).length;
    ...
    renderEventRow(e, idx) {
        ...
        const buyLabel = e.isSoldOut ? 'Detalii' : 'CumpÄƒrÄƒ';
    ...
    renderEventsSubheader(events) {
        ...
        // the empty-state card already says "Nu existÄƒ evenimente
        // viitoare", so the header would be redundant â€” hide it entirely.
        ...
        const last = lastEvent && lastEvent.date instanceof Date && !isNaN(lastEvent.date)
            ...
            : null;
        el.textContent = events.length + (events.length === 1 ? ' eveniment' : ' evenimente') + (last ? ' Â· pÃ¢nÄƒ Ã®n ' + last : '');
    },
    ...
    /* â•â•â•â•â•â•â•â•â•â•â•â•â• QUICK INFO (sidebar) â•â•â•â•â•â•â•â•â•â•â•â•â• */
    renderQuickInfo(venue) {
        ...
        // Social buttons â€” full-width rows with icon + platform label so each
        ...
        if (rows.length === 0) {
            container.innerHTML = '<div class="p-5 text-sm italic text-slate-500">InformaÈ›ii de contact indisponibile</div>';
            return;
        }
    ...
    /* â•â•â•â•â•â•â•â•â•â•â•â•â• ADDRESS + MAP â•â•â•â•â•â•â•â•â•â•â•â•â• */
    renderAddress(venue) {
    ...
    renderMap(venue) {
        ...
        if (mapQuery) {
            ...
            mapContainer.innerHTML =
                ...
                '<p class="text-sm mt-2">LocaÈ›ie indisponibilÄƒ</p>';
        }
    },
    ...
    /* â•â•â•â•â•â•â•â•â•â•â•â•â• MOBILE STICKY CTA â•â•â•â•â•â•â•â•â•â•â•â•â• */
    renderMobileStickyCta(events) {
        ...
        label.textContent = first.title + (when ? ' Â· ' + when + time : '');
        link.href = '/bilete/' + first.slug;
        bar.classList.remove('hidden');
    },

    /* â•â•â•â•â•â•â•â•â•â•â•â•â• SIMILAR VENUES â•â•â•â•â•â•â•â•â•â•â•â•â• */
    renderSimilarVenues(venues) {
        ...
        if (titleEl) {
            const city = this.venue && this.venue.city;
            titleEl.textContent = city ? 'LocaÈ›ii similare Ã®n ' + city : 'LocaÈ›ii similare';
        }
        if (subtitleEl) {
            const cat = this.venue && this.venue.primaryCategory && this.venue.primaryCategory.name;
            subtitleEl.textContent = cat ? 'Alte ' + cat.toLowerCase() + ' din zona ta' : 'Alte locaÈ›ii pe care le poÈ›i descoperi';
        }
        ...
        container.innerHTML = venues.slice(0, 4).map(function (v, i) {
            ...
            return '<a href="/locatie/' + self.escapeAttr(v.slug) + '" class="bg-white rounded-2xl venue-card-shadow venue-card-shadow-hover overflow-hidden group transition">' +
                ...
                '<div class="p-3">' +
                    ...
                    '<div class="text-xs text-slate-500 mt-0.5">' + self.escapeHtml(v.city) + (v.eventsCount ? ' Â· ' + v.eventsCount + ' evenimente' : '') + '</div>' +
                '</div>' +
            '</a>';
        }).join('');
    },

    /* â•â•â•â•â•â•â•â•â•â•â•â•â• SHARE DROPDOWN â•â•â•â•â•â•â•â•â•â•â•â•â• */
    setupShareLinks() {
        ...
        const text = this.venue ? ('DescoperÄƒ evenimentele de la ' + this.venue.name + ' pe AmBilet.ro') : '';
    ...
    copyShareLink() {
        ...
        const done = function () {
            ...
            setTimeout(function () {
                if (label) label.textContent = 'CopiazÄƒ link';
                if (hint) hint.textContent = 'CopiazÄƒ URL-ul paginii';
    ...
    /* â•â•â•â•â•â•â•â•â•â•â•â•â• FOLLOW â•â•â•â•â•â•â•â•â•â•â•â•â• */
    async loadFollowStatus() {
        if (!this.venueSlug) return;
        try {
            const response = await BileteOnlineAPI.checkVenueFavorite(this.venueSlug);
            if (response.success && response.data) {
    ...
    async toggleFollow() {
        if (!this.venueSlug) return;
        if (!BileteOnlineAuth.isLoggedIn()) {
            window.location.href = '/autentificare?redirect=' + encodeURIComponent(window.location.pathname);
            return;
        }
        try {
            const response = await BileteOnlineAPI.toggleVenueFavorite(this.venueSlug);
            if (response.success && response.data) {
                this.isFollowing = response.data.is_favorite;
                this.updateFollowButton();
            }
    ...
    updateFollowButton() {
        ...
        if (this.isFollowing) {
            ...
            text.textContent = 'UrmÄƒreÈ™te locaÈ›ia';
        }
    },

    /* â•â•â•â•â•â•â•â•â•â•â•â•â• CONTACT MODAL â•â•â•â•â•â•â•â•â•â•â•â•â• */
    openContactModal() {
    ...
    async submitContactForm(event) {
        ...
        const data = {
            ...
            // Honeypot â€” hidden field that bots auto-fill; humans never see
            // it. Server drops the submission silently when it carries a value.
            website_url: hpField ? hpField.value : ''
        };

        try {
            const response = await BileteOnlineAPI.post('/venues/' + this.venueSlug + '/contact', data);
            if (response.success) {
                if (successEl) {
                    successEl.textContent = 'Mesajul tÄƒu a fost trimis cu succes! LocaÈ›ia va reveni cu un rÄƒspuns.';
                    successEl.classList.remove('hidden');
                }
                form.reset();
                setTimeout(function () { VenuePage.closeContactModal(); }, 3000);
            } else if (errorEl) {
                errorEl.textContent = response.message || 'Eroare la trimiterea mesajului. ÃŽncearcÄƒ din nou.';
                errorEl.classList.remove('hidden');
            }
        } catch (e) {
            if (errorEl) {
                let msg = 'Eroare la trimiterea mesajului. ÃŽncearcÄƒ din nou.';
    ...
    buildStars(rating) {
        ...
        let out = '';
        for (let i = 1; i <= 5; i++) out += i <= r ? 'â˜…' : 'â˜†';
        return out;
    },
    shuffle(arr) {
        // Fisherâ€“Yates in place. Returns the same array for chaining.
    ...
    formatReviewDate(value) {
        if (!value) return '';
        // If already a human-readable string (e.g. "o sÄƒptÄƒmÃ¢nÄƒ Ã®n urmÄƒ"), return as-is
        if (typeof value === 'string' && !/^\d+$/.test(value) && !/^\d{4}-\d{2}-\d{2}/.test(value)) return value;
        // Unix timestamp (seconds) â€” Google Places returns integer
        let d;
