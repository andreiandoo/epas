 ...
 *
 * Dependencies: BileteOnlineAPI, BileteOnlineEventCard, BileteOnlinePagination, BileteOnlineEmptyState, BileteOnlineDataTransformer
 */
...
const GenrePage = {
    ...
    async loadArtists() {
        ...
        try {
            const response = await BileteOnlineAPI.get('/artists?genre=' + this.genre + '&limit=6');
    ...
    renderArtistCard(artist) {
        const name = BileteOnlineEventCard.escapeHtml(artist.name || 'Artist');
    ...
    async loadSubgenres() {
        ...
        try {
            const response = await BileteOnlineAPI.get('/subgenres?genre=' + this.genre);
            if (response.data && response.data.length > 0) {
                ...
                if (container) {
                    ...
                    response.data.forEach(sub => {
                        container.innerHTML += '<button class="px-4 py-2 text-sm font-medium transition-all border rounded-full bg-surface border-border hover:bg-primary hover:text-white hover:border-primary" data-subgenre="' + sub.slug + '">' + BileteOnlineEventCard.escapeHtml(sub.name) + '</button>';
                    });
                }
            } else {
                section.style.display = 'none';
    ...
    async loadCities() {
        try {
            ...
            const response = await BileteOnlineAPI.get('/events/cities?genre=' + this.genre);
            ...
            if (cities.length > 0) {
                ...
                if (select) {
                    ...
                    cities.forEach(city => {
                        select.innerHTML += '<option value="' + (city.slug || city.name) + '">' + BileteOnlineEventCard.escapeHtml(city.name) + '</option>';
                    });
                }
    ...
    async loadEvents(append = false) {
        ...
        if (!append) {
            ...
            container.innerHTML = BileteOnlineEventCard.renderSkeletons(8);
        }
        ...
        try {
            ...
            if (response.data) {
                ...
                if (events.length > 0) {
                    ...
                    container.innerHTML = BileteOnlineEmptyState.noEvents({
            ...
            container.innerHTML = BileteOnlineEmptyState.error({
    ...
    renderMonthGroup(group) {
        const eventsHtml = BileteOnlineEventCard.renderMany(group.events);
    ...
    loadMore() {
        this.currentPage++;
        BileteOnlinePagination.setLoadingState(true);
        this.loadEvents(true).finally(() => {
            BileteOnlinePagination.setLoadingState(false);
        });
    },
