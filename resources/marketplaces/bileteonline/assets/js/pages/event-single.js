 ...
 *
 * Dependencies: BileteOnlineAPI, BileteOnlineCart, BileteOnlineEventCard, BileteOnlineDataTransformer
 */
...
const EventPage = {
     ...
     * restore the page from bfcache â€” JS memory is preserved but the cart
    ...
    async trackView() {
        try {
            var response = await BileteOnlineAPI.trackEventView(this.slug);
    ...
    async loadInterestStatus() {
        ...
        if (typeof BileteOnlineAuth !== 'undefined' && !BileteOnlineAuth.isLoggedIn()) {
            console.log('[EventPage] User not logged in, skipping interest check');
            return;
        }

        try {
            var response = await BileteOnlineAPI.checkEventInterest(this.slug);
            ...
            if (response.success && response.data) {
                ...
                if (response.data.views_count !== undefined) {
                    document.getElementById(this.elements.eventViews).textContent =
                        this.formatCount(response.data.views_count) + ' vizualizÄƒri';
                }
            }
        } catch (e) {
    ...
    async toggleInterest() {
        ...
        try {
            var response = await BileteOnlineAPI.toggleEventInterest(this.slug);
     ...
     */
    async loadEvent() {
        // Use server-injected data if available (eliminates browserâ†’API roundtrip)
        ...
        try {
            ...
            const response = await BileteOnlineAPI.getEvent(this.slug, params);
            if (response.success && response.data) {
                ...
                this.showError('Eveniment negÄƒsit');
            }
        } catch (error) {
            ...
            if (error.status === 410 && error.data && error.data.ended) {
                ...
                this.showError('Eroare la Ã®ncÄƒrcarea evenimentului');
            }
        }
    },
        ...
        el.innerHTML =
            '<div class="w-full max-w-md mx-auto py-16 px-4 text-center">' +
                '<div class="mb-8">' +
                    ...
                    '<p class="text-muted">Acest eveniment este protejat cu parolÄƒ. Introdu parola pentru a accesa pagina.</p>' +
                '</div>' +
                '<div id="password-gate-form">' +
                    '<div class="flex gap-2">' +
                        ...
                        '</button>' +
                    '</div>' +
                    '<p id="password-error" class="mt-3 text-sm text-red-600 hidden">ParolÄƒ incorectÄƒ. ÃŽncearcÄƒ din nou.</p>' +
                '</div>' +
            '</div>';
    ...
    async submitPassword() {
        ...
        if (btn) {
            btn.disabled = true;
            btn.textContent = 'Se verificÄƒ...';
        }
        if (errorEl) errorEl.classList.add('hidden');

        try {
            const response = await BileteOnlineAPI.verifyEventPassword(this.slug, password);
            if (response.success) {
    ...
    showEndedPerformance(data) {
        ...
        var days = ['DuminicÄƒ', 'Luni', 'MarÈ›i', 'Miercuri', 'Joi', 'Vineri', 'SÃ¢mbÄƒtÄƒ'];

        var perfsHtml = '';
        var upcoming = data.upcoming_performances || [];
        if (upcoming.length > 0) {
            perfsHtml = '<div class="mt-6 text-left">' +
                '<h3 class="mb-3 text-lg font-bold text-secondary">Mai poÈ›i gÄƒsi bilete la:</h3>' +
                '<div class="space-y-2">' +
                upcoming.map(function(p) {
                    ...
                    var timeRange = p.start_time + (p.end_time ? ' â€“ ' + p.end_time : '');
        ...
        el.innerHTML =
            '<div class="w-full py-12 text-center">' +
                ...
                '<h1 class="mb-2 text-2xl font-bold text-secondary">AceastÄƒ reprezentaÈ›ie s-a Ã®ncheiat</h1>' +
    ...
    async loadErrorRecommendations() {
        try {
            const response = await BileteOnlineAPI.getFeaturedEvents(8);
            ...
            container.innerHTML =
                ...
                '<div class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">' +
                    BileteOnlineEventCard.renderMany(events.slice(0, 8), {
                        urlPrefix: '/bilete/',
        ...
        banner.innerHTML =
            '<div class="bg-primary text-white rounded-2xl p-6 mobile:rounded-none">' +
                '<div class="flex items-center justify-center gap-3">' +
                    '<h2 class="text-xl font-bold">Evenimentul s-a Ã®ncheiat</h2>' +
                '</div>' +
                '<p class="mb-6 text-center">dar Ã®ncÄƒ mai gÄƒseÈ™ti bilete la:</p>' +
                '<div id="ended-related-events" class="grid gap-4 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"></div>' +
                '<div id="ended-related-loading" class="py-8 text-sm text-center">Se cautÄƒ sugestii de evenimente...</div>' +
            '</div>';
    ...
    async loadEndedRelatedEvents() {
        try {
            ...
            const response = await BileteOnlineAPI.get('/events?' + params.toString());
            ...
            if (response.success && response.data?.length) {
                ...
                if (filtered.length > 0 && container) {
                    container.innerHTML = BileteOnlineEventCard.renderMany(filtered, {
                        showCategory: true,
                        showPrice: true,
                        showVenue: true,
                        urlPrefix: '/bilete/'
    ...
    render() {
        ...
        // Update page title
        document.title = e.title + ' â€” ' + (typeof BILETEONLINE_CONFIG !== 'undefined' ? BILETEONLINE_CONFIG.SITE_NAME : 'bilete.online');
        ...
        if (this.isPreview) {
            ...
            if (titleEl) {
                ...
                const title = isTestMode ? 'Mod test comandÄƒ activ' : 'Previzualizare';
                const description = isTestMode
                    ? 'Comenzile plasate vor fi gratuite È™i marcate ca test. Nu se proceseazÄƒ plÄƒÈ›i.'
                    : 'Acest eveniment nu este Ã®ncÄƒ public. Aceasta este o previzualizare.';
        ...
        // Time â€” for multi-day, use selected performance's times
        ...
        // Venue (with city)
        var venueName = e.venue?.name || e.location || 'LocaÈ›ie TBA';
        ...
        document.getElementById(this.elements.eventViews).textContent = this.formatCount(e.views_count || 0) + ' vizualizÄƒri';

        // Performance selector (multi-day events) â€” insert BEFORE description
        ...
        // Venue section
        this.renderVenue(e.venue || { name: e.location || 'LocaÈ›ie TBA' });
        ...
        // Related events â€” lazy-load when section scrolls into view
        if (!this.eventEnded) {
    ...
    renderBadges(e) {
        ...
        if (e.is_cancelled) {
            ...
            badgesHtml.push('<span class="px-3 py-1.5 bg-gray-600 text-white text-xs font-bold rounded-lg uppercase">ÃŽNCHEIAT</span>');
    ...
    renderDate(e) {
        ...
        if (durationMode === 'range' && e.range_start_date && e.range_end_date) {
            ...
            if (remainingCount <= 0) {
                ...
                document.getElementById(this.elements.eventDateFull).textContent =
                    headerDate.getDate() + ' ' + months[headerDate.getMonth()] + ' ' + headerDate.getFullYear() +
                    ' (+' + remainingCount + ' ' + (remainingCount === 1 ? 'altÄƒ datÄƒ' : 'alte date') + ')';
            }
        ...
        requestAnimationFrame(function() {
            ...
            btn.addEventListener('click', function() {
                ...
                if (collapsed) {
                    ...
                    textEl.textContent = 'Vezi mai puÈ›in';
                } else {
    ...
    formatDescription(desc) {
        if (!desc) return '<p class="text-muted">Descriere indisponibilÄƒ</p>';
            ...
            allHtml += '<div class="flex flex-col gap-6 md:flex-row ' + containerClass + (i > 0 ? ' pt-6 mt-6' + (isHeadliner ? '' : ' border-t border-border') : '') + '">' +
                '<div class="' + imageColClass + '">' +
                    '<a href="' + artistLink + '" class="relative block">' +
                        ...
                        (isHeadliner ? '<div class="absolute px-2 py-1 text-xs font-bold text-white rounded-lg shadow-lg top-3 left-3 bg-gradient-to-r from-amber-500 to-amber-600">â˜… HEADLINER</div>' : '') +
                    '</a>' +
                '</div>' +
                '<div class="' + contentColClass + '">' +
                    ...
                    '</div>' +
                    (artistDescription ? '<p class="mb-2 leading-relaxed text-muted">' + artistDescription + '</p>' : '<p class="mb-4 leading-relaxed text-muted">Detalii despre artist vor fi disponibile Ã®n curÃ¢nd.</p>') +
    ...
    renderPerformanceList() {
        ...
        section.innerHTML =
            '<h3 style="font-size:15px;font-weight:700;margin-bottom:10px;color:#e2e8f0;">Acest eveniment are mai multe reprezentaÈ›ii. Alege-o pe cea care È›i se potriveÈ™te cel mai bine.</h3>' +
            ...
            performances.map(function(p) {
                ...
                const timeRange = time + (endTime ? ' â€“ ' + endTime : '');
        ...
        section.querySelectorAll('.perf-list-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                if (perf) {
                    ...
                    const w = ['DuminicÄƒ', 'Luni', 'MarÈ›i', 'Miercuri', 'Joi', 'Vineri', 'SÃ¢mbÄƒtÄƒ'];
    ...
    showPerformancePicker() {
        ...
        var days = ['Dum', 'Lun', 'Mar', 'Mie', 'Joi', 'Vin', 'SÃ¢m'];
        ...
        var listHtml = performances.map(function(p) {
            ...
            var endTime = p.end_time ? ' â€“ ' + p.end_time : '';
        ...
        modal.innerHTML =
            ...
            '<div class="relative w-full max-h-[80vh] bg-white rounded-t-2xl shadow-xl overflow-hidden">' +
                '<div class="flex items-center justify-between px-5 py-4 border-b border-border">' +
                    '<h3 class="text-base font-bold text-secondary">SelecteazÄƒ reprezentaÈ›ia</h3>' +
    ...
    renderTicketTypes() {
        ...
        var ticketCards = this.ticketTypes.map(function(tt) {
            ...
            if (eventIsCancelled) {
                ...
                availabilityHtml = '<span class="text-xs font-semibold text-primary">ðŸ”¥ Doar ' + tt.available + ' disponibile</span>';
            ...
            if (eventIsCancelled) {
                ...
                // picker â€” exposing +/- here lets users bump qty past their
                ...
                if (selectedCount > 0) {
                    ...
                    seatedBtnLabel = 'Alege locurile pe hartÄƒ';
                }
    ...
    updateQuantity(ticketId, delta) {
            ...
            if (newQty > maxPerOrder) {
                if (currentQty < maxPerOrder) {
                    ...
                    if (typeof BileteOnlineNotifications !== 'undefined') {
                        BileteOnlineNotifications.warning('Maxim ' + maxPerOrder + ' bilete de acest tip per comandÄƒ');
                    }
                    return;
                }
            }
        ...
        if (newQty !== currentQty) {
            ...
            // If transitioning to/from 0, re-render to swap between "AdaugÄƒ" button and qty controls
    ...
    updateCheckoutButton() {
        ...
        if (needingSelection.length > 0) {
            ...
            // Ready to checkout
            btnText.textContent = 'CumpÄƒrÄƒ bilete';
            if (btnIcon) {
    ...
    handleCheckout() {
        ...
        if (hasSelectedSeats) {
            ...
            // via bfcache without touching anything) â€” no need to re-hold,
            // just navigate.
            if (this.areSelectedSeatsInCart()) {
                window.location.href = '/cos';
                return;
            }
            // Otherwise (cart was emptied, or selection diverged) â€” clear any
        ...
        // No seating involved â€” original flow.
        this.addToCart();
    },
    ...
    addToCart() {
        ...
        if (this.event.duration_mode === 'multi_day' && this.event.performances && this.event.performances.length > 1 && !this.event.selectedPerformanceId) {
            if (typeof BileteOnlineNotifications !== 'undefined') {
                BileteOnlineNotifications.warning('Te rugÄƒm sÄƒ alegi o reprezentare Ã®nainte de a adÄƒuga bilete Ã®n coÈ™.');
            } else {
                alert('Te rugÄƒm sÄƒ alegi o reprezentare Ã®nainte de a adÄƒuga bilete Ã®n coÈ™.');
            }
        ...
        for (var ticketId in this.quantities) {
            ...
            if (qty > 0) {
                ...
                if (tt) {
                    ...
                    BileteOnlineCart.addItem(self.event.id, eventData, tt.id, ticketTypeData, qty);
                    addedAny = true;
                }
            }
        }
    ...
    renderTourEvents(tourEvents, tourName, tourType) {
        ...
        if (tourName && nameDisplay) {
            ...
            nameFallback.textContent = isTour
                ? 'Evenimentul face parte dintr-un turneu. Alege È™i alte date.'
                : 'Evenimentul face parte dintr-o serie. Alege È™i alte date.';
        }
    ...
    /**
     * Render related events grid using BileteOnlineEventCard component
     */
    renderRelatedEvents(events) {
        ...
        // Use BileteOnlineEventCard component for consistent rendering
        document.getElementById(this.elements.relatedEvents).innerHTML = BileteOnlineEventCard.renderMany(events, {
            showCategory: true,
            showPrice: true,
            showVenue: true,
            urlPrefix: '/bilete/'
     ...
     */
    updateHeaderCart() {
        const count = typeof BileteOnlineCart !== 'undefined' ? BileteOnlineCart.getItemCount() : 0;
    ...
    async refreshSeatStatuses() {
        ...
        try {
            const response = await BileteOnlineAPI.request('/marketplace-events/' + this.slug, { method: 'GET', noCache: true });
            ...
            // Build a seat_uid â†’ status map from the fresh payload
            const freshStatus = {};
    ...
    async openSeatSelection(ticketTypeId) {
        ...
        // Don't wipe existing selections â€” the cart-restore flow puts
        ...
        // (Originally the clear was here to start fresh â€” we accomplish
        // that now via "ReseteazÄƒ selecÈ›ia" button if/when needed.)
    ...
    createSeatSelectionModal() {
            ...
            '<div class="absolute inset-0 md:inset-4 lg:inset-8 bg-white md:rounded-2xl shadow-2xl flex flex-col overflow-hidden">' +
                ...
                '<div class="flex items-center justify-between px-4 md:px-6 py-3 border-b border-border mobile:bg-primary">' +
                    '<div class="flex-1 min-w-0">' +
                        ...
                        '<p class="text-xs md:text-sm text-muted mobile:text-white" id="seat-selection-subtitle">SelecteazÄƒ locurile dorite pe hartÄƒ</p>' +
                    '</div>' +
                    '<button onclick="EventPage.closeSeatSelection()" aria-label="ÃŽnchide" class="p-2 transition-colors rounded-lg hover:bg-surface flex-shrink-0" aria-label="ÃŽnchide">' +
                ...
                '<div class="flex-1 flex overflow-hidden relative">' +
                    ...
                    '<div class="flex-1 flex flex-col bg-surface/50 overflow-hidden">' +
                        ...
                        '<div class="flex items-center justify-between px-3 md:px-4 py-2 bg-white border-b border-border">' +
                            ...
                            '<div class="text-xs text-muted md:hidden">CiupeÈ™te pentru zoom</div>' +
                        '</div>' +
                    ...
                    '<div class="w-64 lg:w-80 border-l border-border bg-white overflow-y-auto hidden lg:flex flex-col mobile:border-l-0 mobile:border-r" id="seat-selected-sidebar">' +
                        '<div class="py-4 px-3 border-b border-border flex items-center justify-between mobile:h-[49px] mobile:py-0 mobile:px-4">' +
                            ...
                            '<button type="button" id="seat-selected-close" class="lg:hidden p-1 rounded hover:bg-surface" aria-label="ÃŽnchide">' +
                        ...
                        '<div class="p-3 border-t border-border bg-surface/30 mobile:hidden">' +
                            ...
                            '</div>' +
                            '<div class="flex gap-2">' +
                                '<button onclick="EventPage.confirmSeatSelection()" class="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-primary rounded-xl hover:bg-primary-dark transition-colors" name="buy-tickets">CumpÄƒrÄƒ bilete</button>' +
                                '<button onclick="EventPage.clearAllSelections()" aria-label="GoleÈ™te coÈ™ul" class="p-2.5 text-secondary border border-secondary rounded-xl opacity-50 hover:bg-red-50 hover:opacity-100 hover:border-red-600 hover:text-red-600 transition-all" title="GoleÈ™te coÈ™ul" aria-label="GoleÈ™te coÈ™ul">' +
                ...
                '<div class="lg:hidden border-t border-border bg-white px-4 py-3 flex items-center justify-between gap-3" id="seat-mobile-bottom-bar">' +
                    ...
                    '<div class="flex gap-2 flex-shrink-0">' +
                        '<button onclick="EventPage.clearAllSelections()" aria-label="GoleÈ™te" class="p-2.5 text-muted border border-gray-200 rounded-xl hover:bg-red-50 hover:border-red-300 hover:text-red-600 transition-all" title="GoleÈ™te" aria-label="GoleÈ™te">' +
                            '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>' +
                        '</button>' +
                        '<button onclick="EventPage.confirmSeatSelection()" class="px-5 py-2.5 text-sm font-semibold text-white bg-primary rounded-xl hover:bg-primary-dark transition-colors" id="mobile-seats-buy-btn" aria-label="CumpÄƒrÄƒ bilete">CumpÄƒrÄƒ bilete</button>' +
                    '</div>' +
                '</div>' +
            '</div>';
        ...
        // Setup zoom with mouse wheel â€” anchored at cursor position
        ...
        mapWrapper.addEventListener('touchmove', function(e) {
            if (e.touches.length === 2) {
                // Pinch zoom â€” anchor at midpoint between the two fingers
                e.preventDefault();
                var dx = e.touches[0].clientX - e.touches[1].clientX;
                var dy = e.touches[0].clientY - e.touches[1].clientY;
    ...
    renderSelectedTicketsPanel() {
            ...
            seats.forEach(function(seat, index) {
                html += '<div class="flex items-center justify-between p-2 bg-surface rounded-lg mb-1">' +
                    '<div class="text-xs">' +
                        ...
                        '<div class="text-muted">RÃ¢nd ' + seat.row + ', Loc ' + seat.seat + '</div>' +
                    '</div>' +
                    '<button onclick="EventPage.removeSeat(\'' + tt.id + '\', ' + index + ')" aria-label="È˜terge" class="p-1 text-red-500 hover:bg-red-50 rounded transition-colors" title="È˜terge" aria-label="È˜terge">' +
        ...
        container.innerHTML = html || '<p class="text-xs text-muted text-center py-4">SelecteazÄƒ locuri pe hartÄƒ</p>';
    ...
    renderDecorativeSectionSvg(section) {
        ...
        if (shape === 'polygon' && metadata.points) {
            ...
            // Draw text label â€” centered vertically in section bounds
                ...
                section.rows.forEach(function(row) {
                    ...
                    row.seats.forEach(function(seat) {
                        ...
                        if (isDisabled || status === 'blocked') {
                            ...
                            tooltipText = section.name + ', RÃ¢nd ' + row.label + ', Loc ' + seat.label;
                            if (!isAllowed) tooltipText += ' â€” indisponibil pentru acest bilet';
                            else if (status === 'sold') tooltipText += ' â€” vÃ¢ndut';
                            else if (status === 'held') tooltipText += ' â€” rezervat';
                        }
    ...
    renderSeatingMapAllSections(assignedSectionIds) {
        ...
        layout.sections.forEach(function(section) {
            ...
            // Section-wide seat X bounds and gap â€” used to align row labels on a
            ...
            if (section.rows) {
                section.rows.forEach(function(row) {
                    ...
                    // Row-based ticket type lookup (keep original order â€” first assigned type determines color)
                    ...
                    row.seats.forEach(function(seat) {
                        ...
                        if (isDisabled || status === 'blocked') {
                            ...
                            if (!isRowAssigned) {
                                ...
                                tooltipText += ' â€” vÃ¢ndut';
                            } else if (status === 'held') {
                                tooltipText += ' â€” rezervat';
                            }
                        }
    ...
    isSeatSelected(ticketTypeId, seatId, seatUid) {
        ...
        return seats.some(function(s) {
            // Compare on both id and seat_uid â€” cart-restored selections
    ...
    toggleSeatAuto(rowId, seatId, sectionName, rowLabel, seatLabel, seatUid) {
        ...
        // First check if seat is already selected in any ticket type â€” if so, deselect
    ...
    showTicketTypeChooser(ticketTypes, seatId, sectionName, rowLabel, seatLabel, seatUid) {
        ...
        html += '<div class="text-xs text-gray-500 mb-2">' + sectionName + ', RÃ¢nd ' + rowLabel + ', Loc ' + seatLabel + '</div>';
        html += '<div class="text-xs text-blue-600 bg-blue-50 rounded-lg px-3 py-2 mb-3">Acest loc are alocate mai multe tipuri de bilete. Alege-l pe cel care È›i se potriveÈ™te.</div>';
        ...
        html += '<button onclick="document.getElementById(\'tt-chooser-popup\').remove()" class="w-full text-center text-xs text-gray-400 mt-2 py-1 hover:text-gray-600 transition-colors" aria-label="AnuleazÄƒ">AnuleazÄƒ</button>';
        html += '</div>';
    ...
    updateSeatSelectionSummary() {
        ...
        if (mobileBuyBtn) {
            mobileBuyBtn.textContent = totalSeats > 0 ? 'CumpÄƒrÄƒ bilete' : 'SelecteazÄƒ locuri';
            mobileBuyBtn.disabled = totalSeats === 0;
            mobileBuyBtn.style.opacity = totalSeats > 0 ? '1' : '0.5';
        }
        // Show "Vezi bilete â†‘" hint on the bottom-bar summary only after at least
        ...
        if (seatDetails.length > 0) {
            var details = seatDetails.map(function(d) {
                return d.section + ', rÃ¢nd ' + d.row + ', loc ' + d.seat;
            }).slice(0, 3).join('; ');
            if (seatDetails.length > 3) {
                details += ' È™i Ã®ncÄƒ ' + (seatDetails.length - 3) + ' locuri';
            }
     ...
     * at zero even though the cart icon shows N items â€” when the user
    ...
    restoreSelectionFromCart() {
        if (!this.event || !this.event.id || typeof BileteOnlineCart === 'undefined') return;

        var eventId = this.event.id;
        var self = this;

        // Reset state â€” cart is the source of truth on (re)mount / bfcache
        ...
        var items = BileteOnlineCart.getItems();
    ...
    areSelectedSeatsInCart() {
        if (!this.event || !this.event.id || typeof BileteOnlineCart === 'undefined') return false;
        var eventId = this.event.id;
        var ticketTypeIds = Object.keys(this.selectedSeats || {});
        if (ticketTypeIds.length === 0) return false;

        var items = BileteOnlineCart.getItems().filter(function(it) { return it.eventId === eventId; });
    ...
    renderCartSeatBanner() {
        ...
        if (!this.event || !this.event.id || typeof BileteOnlineCart === 'undefined') {
            container.innerHTML = '';
            return;
        }

        var eventId = this.event.id;
        var items = BileteOnlineCart.getItems().filter(function(item) {
        ...
        var seatRows = items.map(function(item) {
            ...
            var seatList = item.seats.map(function(s) {
                ...
                return [section, row ? 'R' + row : '', seat ? 'L' + seat : ''].filter(Boolean).join(' Â· ');
            }).join('<br>');
            return ''
        ...
        container.innerHTML = ''
            ...
            +     '<div style="flex:1;">'
            +       '<div style="font-weight:700;color:#065f46;margin-bottom:6px;">Ai locuri rezervate Ã®n coÈ™</div>'
            +       seatRows
            +       '<div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">'
            +         '<a href="/checkout" style="display:inline-flex;align-items:center;gap:6px;padding:6px 14px;font-size:13px;font-weight:600;color:#fff;background:#059669;border-radius:8px;text-decoration:none;">ContinuÄƒ plata</a>'
            +         '<a href="/cos" style="display:inline-flex;align-items:center;gap:6px;padding:6px 14px;font-size:13px;font-weight:600;color:#065f46;background:#fff;border:1px solid #a7f3d0;border-radius:8px;text-decoration:none;">Vezi coÈ™ul</a>'
            +       '</div>'
            +     '</div>'
            +   '</div>'
            + '</div>';
    ...
    clearEventFromCart() {
        ...
        if (filteredItems.length !== items.length) {
            BileteOnlineCart.save(filteredItems);
    ...
    async addToCartWithSeats() {
        ...
        var cart = BileteOnlineCart.getCart();
        ...
        for (var i = 0; i < ticketTypeIds.length; i++) {
            ...
            if (eventSeatingId && seatUids.length > 0) {
                try {
                    ...
                    var response = await BileteOnlineAPI.post('/cart/items/with-seats', apiPayload);
                    ...
                    if (response.success && response.data) {
                        ...
                        // Handle API error â€” surface the server's message so
                    ...
                    // BileteOnlineAPI throws APIError for non-2xx responses â€” its
                    ...
                    var serverMsg = (error && typeof error.message === 'string' && error.message.length > 0 && error.message !== 'An error occurred')
                        ? error.message
                        : 'A apÄƒrut o eroare la rezervarea locurilor. VÄƒ rugÄƒm Ã®ncercaÈ›i din nou.';
                    ...
                    if (Array.isArray(unavailable) && unavailable.length > 0) {
                        var labels = unavailable.map(function (uid) {
                            ...
                            return match ? (match.section + ' Â· RÃ¢nd ' + match.row + ' Â· Loc ' + match.seat) : uid;
                        });
        ...
        BileteOnlineCart.save(cart.items);

        // Start reservation timer (15 minutes)
        if (typeof BileteOnlineCart.startReservationTimer === 'function') {
            BileteOnlineCart.startReservationTimer();
        }

        // CAPI AddToCart for seated flow (Layer B bridge â€” backend forwards
        // to Meta Graph API). The seated flow bypasses BileteOnlineCart.addItem
        ...
        console.log('[EventPage] Cart after adding seats:', BileteOnlineCart.getCart());
    },
    ...
    async loadOrganizerTracking(organizerId) {
        try {
            var response = await fetch(
                window.BILETEONLINE.apiUrl + '?action=tracking.organizer-scripts&organizer_id=' + organizerId
            );
