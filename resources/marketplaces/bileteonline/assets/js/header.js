(function() {
    ...
    function renderMockResults(query) {
        var mockData = {
            ...
            locations: [
                { name: 'Arena ' + query, slug: 'arena-demo', city: 'BucureÈ™ti', image: null }
            ]
        };
    ...
    var CART_STORAGE_KEY = 'bileteonline_cart';
    ...
    function updateItemQuantity(index, action) {
        ...
        // When quantity drops to 0, go through BileteOnlineCart.removeItem so seats get released
        if (newQty <= 0 && window.BileteOnlineCart && typeof window.BileteOnlineCart.removeItem === 'function' && item.key) {
            window.BileteOnlineCart.removeItem(item.key);
            updateCartUI();
            return;
        }
    ...
    function removeCartItem(index) {
        ...
        // Delegate to BileteOnlineCart.removeItem so seats are released via API
        if (window.BileteOnlineCart && typeof window.BileteOnlineCart.removeItem === 'function' && item && item.key) {
            window.BileteOnlineCart.removeItem(item.key);
            updateCartUI();
            return;
        }
    ...
    window.addEventListener('bileteonline:cart:update', function() { updateCartUI(); });
    window.addEventListener('bileteonline:cart:expired', function() { closeCartDrawer(); updateCartUI(); });
    window.addEventListener('bileteonline:cart:clear', function() { updateCartUI(); });

    if (window.BileteOnlineCart) {
        window.BileteOnlineCart.openDrawer = openCartDrawer;
        window.BileteOnlineCart.closeDrawer = closeCartDrawer;
        window.BileteOnlineCart.updateUI = updateCartUI;
    } else {
        window.BileteOnlineCart = { openDrawer: openCartDrawer, closeDrawer: closeCartDrawer, updateUI: updateCartUI, getCart: getCart };
    }
    ...
    (function initHeaderTimerBar() {
        ...
        function updateHeaderTimer() {
            var cart = [];
            try {
                var stored = localStorage.getItem('bileteonline_cart');
        ...
        setTimeout(function() {
            updateHeaderTimer();
            var savedEndTime = localStorage.getItem('cart_end_time');
            var storedCart = localStorage.getItem('bileteonline_cart');
        ...
        window.addEventListener('bileteonline:cart:update', function() {
            updateHeaderTimer();
            if (!timerInterval) {
                var storedCart = localStorage.getItem('bileteonline_cart');
                if (storedCart) {
        ...
        window.addEventListener('bileteonline:cart:clear', function() {
            showDefaultBar();
            isRed = false;
        ...
        window.addEventListener('bileteonline:cart:expired', function() {
            showDefaultBar();
            isRed = false;
            if (timerInterval) { clearInterval(timerInterval); timerInterval = null; }
        });
