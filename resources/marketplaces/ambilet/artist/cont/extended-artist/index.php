...
<main class="min-h-screen pt-16 lg:ml-64 lg:pt-0">
    <div class="p-4 lg:p-8">
        <header class="flex items-start gap-4 mb-8">
            <div class="flex items-center justify-center shadow-lg h-14 w-14 rounded-2xl bg-gradient-to-br from-primary to-purple-600">
                <svg class="text-white h-7 w-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div id="ea-status-card" class="max-w-3xl mx-auto space-y-6">
            <div class="p-6 text-center bg-white border rounded-2xl border-border text-muted">
        ...
        <div class="max-w-5xl mx-auto mt-10">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <article class="p-5 bg-white border rounded-2xl border-border">
                    <div class="inline-flex items-center justify-center w-10 h-10 mb-3 rounded-xl bg-primary/10 text-primary">
                ...
                <article class="p-5 bg-white border rounded-2xl border-border">
                    <div class="inline-flex items-center justify-center w-10 h-10 mb-3 rounded-xl bg-emerald-500/10 text-emerald-600">
                ...
                <article class="p-5 bg-white border rounded-2xl border-border">
                    <div class="inline-flex items-center justify-center w-10 h-10 mb-3 rounded-xl bg-amber-500/10 text-amber-600">
                ...
                <article class="p-5 bg-white border rounded-2xl border-border">
                    <div class="inline-flex items-center justify-center w-10 h-10 mb-3 rounded-xl bg-rose-500/10 text-rose-600">
...
(function() {
    ...
    if (!token) {
        card.innerHTML = '<div class="p-6 text-center border rounded-2xl border-amber-200 bg-amber-50 text-amber-900">Trebuie sa fii autentificat ca artist pentru a accesa Extended Artist.</div>';
        return;
    }
    ...
    Promise.all([
        ...
        .catch(() => {
            card.innerHTML = '<div class="p-6 text-center text-red-900 border border-red-200 rounded-2xl bg-red-50">Eroare la incarcarea statusului. Reincarca pagina.</div>';
        });
    ...
    function renderStatusCard(el, status, pricing) {
        ...
        if (status.enabled && status.status === 'active') {
            el.innerHTML = `
                <div class="p-6 border rounded-2xl border-emerald-200 bg-gradient-to-br from-emerald-50 to-white">
                    <div class="flex items-start gap-4">
                        <div class="flex items-center justify-center w-12 h-12 text-white rounded-xl bg-emerald-500">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap gap-2 mt-4">
                                <a href="/artist/cont/extended-artist/fan-crm" class="px-4 py-2 text-sm font-semibold text-white rounded-xl bg-primary">Deschide Fan CRM</a>
                                ${status.granted_by === 'self_purchase' ? '<button id="ea-cancel-btn" class="px-4 py-2 text-sm font-semibold text-red-600 bg-white border border-red-300 rounded-xl hover:bg-red-50">Cancelează abonament</button>' : ''}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        if (status.enabled && status.status === 'trial') {
            el.innerHTML = `
                <div class="p-6 border border-blue-200 rounded-2xl bg-gradient-to-br from-blue-50 to-white">
                    <div class="flex items-start gap-4">
                        <div class="flex items-center justify-center w-12 h-12 text-white bg-blue-500 rounded-xl">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap gap-2 mt-4">
                                <a href="/artist/cont/extended-artist/fan-crm" class="px-4 py-2 text-sm font-semibold text-white rounded-xl bg-primary">Deschide Fan CRM</a>
                                <button id="ea-subscribe-btn" class="px-4 py-2 text-sm font-semibold text-blue-700 bg-white border border-blue-400 rounded-xl hover:bg-blue-50">Treci pe abonament (${monthlyPrice} ${currency}/luna)</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        if (status.enabled && status.status === 'cancelled') {
            el.innerHTML = `
                <div class="p-6 border rounded-2xl border-amber-200 bg-amber-50">
                    ...
                    <div class="mt-4">
                        <button id="ea-subscribe-btn" class="px-4 py-2 text-sm font-semibold text-white rounded-xl bg-primary">Reactivează (${monthlyPrice} ${currency}/luna)</button>
                    </div>
                </div>
            `;
            wireSubscribe();
        ...
        el.innerHTML = `
            <div class="p-6 bg-white border rounded-2xl border-border">
                ...
                <div class="flex flex-wrap gap-2 mt-5">
                    ${status.can_start_trial ? `<button id="ea-trial-btn" class="px-4 py-2 text-sm font-semibold text-white rounded-xl bg-primary">Pornește trial gratuit (${trialDays} zile)</button>` : ''}
                    <button id="ea-subscribe-btn" class="px-4 py-2 text-sm font-semibold bg-white border rounded-xl border-border text-secondary hover:bg-surface">${status.can_start_trial ? 'Sari peste trial — abonează-te' : 'Abonează-te'} (${monthlyPrice} ${currency}/luna)</button>
                </div>
            </div>
        `;
        wireTrial();
