...
if (!empty($artistSlug)) {
    ...
    if (!empty($artistData['success']) && !empty($artistData['data'])) {
        ...
        // og:image priority: main hero photo → portrait → logo. The
        // marketplace-client ArtistsController::show emits flat keys
        // (image / portrait / logo) with absolute URLs already
        // resolved by main_image_full_url + friends — no `images.`
        // wrapper. Reading the wrong shape here silently dropped
        // every artist page to og-default.jpg on live.
        $img = $d['image']
            ?? $d['portrait']
            ?? $d['logo']
            ?? null;
    ...
    <section id="spotifySection" class="mt-10">
        <div class="grid items-center grid-cols-1 gap-10 p-8 bg-white lg:grid-cols-2 mobile:p-0">
            <div class="mobile:text-center">
                <h3 class="text-[22px] font-bold text-gray-900 mb-3 flex items-center gap-2.5 mobile:justify-center">
                ...
                <p class="text-gray-500 text-[15px] leading-relaxed mb-5">Descoperă toate albumele, single-urile și colaborările. Peste <span id="spotifyListenersCount"></span> de ascultători lunari!</p>
