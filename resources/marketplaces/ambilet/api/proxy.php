...
function fetchEventsForShareLink($eventIds, $storedTicketData = []) {
    // The dedicated share-stats endpoint is authoritative — it counts the
    ...
    foreach ($eventIds as $eventId) {
        ...
        if ($entry === null) {
            // Upstream failed — fall back to stale snapshot so the page is
...
switch ($action) {
    ...
    case 'organizer.event.leisure.issuers':
        ...
        $endpoint = '/organizer/events/' . $eventId . '/leisure/issuers';
        $requiresAuth = true;
        break;
...
curl_setopt_array($ch, [
    ...
    // network path between the ambilet.ro and core.tixello.com hosts
    // — only the CF route works for this server-to-server hop.
