...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-50 flex flex-col w-64 transform -translate-x-full border-r bg-gradient-to-b from-slate-900 to-slate-800 sidebar lg:static border-slate-700 lg:translate-x-0">
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto">
        ...
        <div class="pt-4 mt-4 border-t border-slate-700">
            ...
            <a href="/organizator/promo" class="sidebar-link <?= $currentPage === 'promo' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group <?= $currentPage !== 'promo' ? 'text-white' : '' ?>">
                <svg class="<?= $currentPage !== 'promo' ? 'text-muted' : 'text-white' ?> w-5 h-5 group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                Coduri promotionale
            </a>
            <a href="/organizator/widget-uri" class="sidebar-link <?= $currentPage === 'widgets' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group <?= $currentPage !== 'widgets' ? 'text-white' : '' ?>">
