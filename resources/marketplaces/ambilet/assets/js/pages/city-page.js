...
const CityPage = {
    ...
    async init(citySlug) {
        ...
        // Paid city-page promotions (is_city_featured) render in the premium
        // section above the grid; the plain listing loads in parallel.
        await Promise.all([
            this.loadCityFeaturedEvents(),
            this.loadEvents()
        ]);
    },
    ...
    /**
     * Load city featured events (is_city_featured — paid city-page promotion).
     * Renders into the premium #cityFeaturedSection above the events grid,
     * mirroring the category page's is_category_featured section.
     */
    async loadCityFeaturedEvents() {
        const section = document.getElementById('cityFeaturedSection');
        const container = document.getElementById('cityFeaturedEvents');
        if (!section || !container || !this.city) return;
        ...
        try {
            ...
            if (events.length > 0) {
                ...
                container.innerHTML = events.map(event => this.renderCityFeaturedCard(event)).join('');
            } else {
                section.classList.add('hidden');
            }
        } catch (e) {
            console.warn('Failed to load city featured events:', e);
            section.classList.add('hidden');
        }
    },

    /**
     * Render premium card for city featured events (mirrors category page)
     */
    renderCityFeaturedCard(event) {
