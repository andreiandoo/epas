        ...
        AmbiletAPI.artist.checkClaim(slug)
            .then((res) => {
                ...
                if (data.is_claimed || data.is_verified || data.is_pending) {
                    if (data.is_verified) {
                        // Profil verificat → arată badge-ul verde din hero
                        // (lângă numele artistului). Nu mai duplicăm pill în
                        // claimCtaContainer, e zgomot vizual.
                        ArtistClaim.showHeroVerifiedBadge();
                    } else if (data.is_pending) {
                        container.innerHTML = ArtistClaim.renderPendingBadge();
                        container.classList.remove('hidden');
                    }
                    // is_claimed=true fără verified/pending → keep hidden.
                    return;
                }
