...
<section class="sticky top-[72px] z-20 py-3 bg-white border-b border-gray-200 shadow-sm lg:hidden">
    <div class="flex items-center justify-between gap-3 px-4">
        <button onclick="openCatFiltersDrawer()" class="flex items-center gap-2 px-4 py-2.5 text-sm font-medium bg-gray-50 border border-gray-200 rounded-xl"  aria-label="Deschide filtrele">
...
<div id="catFiltersDrawer" class="fixed bottom-0 left-0 right-0 z-[110] overflow-hidden transition-transform duration-300 bg-white lg:hidden rounded-t-3xl max-h-[85vh]" style="transform: translateY(100%);">
    <div class="sticky top-0 z-10 flex items-center justify-between p-4 bg-white border-b border-gray-200">
        ...
        <button onclick="closeCatFiltersDrawer()" aria-label="Închide filtrele" class="flex items-center justify-center w-10 h-10 transition-colors bg-gray-100 rounded-full hover:bg-gray-200" aria-label="Închide filtrele">
    ...
    <div class="flex gap-3 p-4 border-t border-gray-200 bg-gray-50">
        <button onclick="clearCatFilters(); closeCatFiltersDrawer();" class="flex-1 px-4 py-3 text-sm font-medium text-gray-700 transition-colors bg-white border border-gray-200 rounded-xl hover:bg-gray-50" aria-label="Șterge filtre">
            Șterge filtre
        </button>
        <button onclick="CategoryPage.loadEvents(); closeCatFiltersDrawer();" class="flex-1 px-4 py-3 text-sm font-bold text-white transition-colors rounded-xl bg-primary hover:bg-primary-dark" aria-label="Aplică filtre">
            Aplică filtre
        </button>
    </div>
</div>
...
<script>document.addEventListener(\'DOMContentLoaded\', () => { CategoryPage.init(' . $categorySlugJS . '); FeaturedCarousel.init({ category: ' . $categorySlugJS . ' }); });</script>';
