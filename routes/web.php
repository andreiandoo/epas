...
// switchSuperAdminToMarketplace() was moved to
// App\Support\SuperAdminMarketplaceSwitcher::switchTo() so it stays defined
// when routes are cached (route:cache skips route files, which left the
// Filament "Login to marketplace" actions calling an undefined function).
...
// AmBilet v2 "next" (ambilet-app2) APK Download — a doua ramura de test
// paralel cu /android-nou. Bundle ID DIFERIT (com.ambilet.scan.next) deci
// se poate instala PE ACELASI telefon alaturi de v1 si v2 (nu inlocuieste
// nimic). Utilizat pentru side-by-side testing intre variante.
Route::get('/download-android-nou-2', function () {
    $path = public_path('downloads/ambilet-android-nou-2.apk');
    if (!file_exists($path)) {
        abort(404, 'APK not available yet');
    }
    return response()->download($path, 'ambilet-android-nou-2.apk', [
        'Content-Type' => 'application/vnd.android.package-archive',
    ]);
})->name('download.android.nou2');

Route::get('/android-nou-2', function () {
    return redirect()->route('download.android.nou2');
});

// Demo payment gateway (test sites) — signed URLs generated by DemoProcessor
Route::get('/demo-pay/{order}', [\App\Http\Controllers\DemoPaymentController::class, 'show'])
    ->name('demo.pay')->middleware('signed');
Route::post('/demo-pay/{order}/complete', [\App\Http\Controllers\DemoPaymentController::class, 'complete'])
    ->name('demo.pay.complete')->middleware('signed');
