    ...
    <main class="max-w-4xl mx-auto px-4 py-6 lg:py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-muted text-sm mt-1">Gestioneaza cardurile salvate</p>
            </div>
        ...
        <!-- Saved Cards -->
        <div class="bg-white rounded-xl lg:rounded-2xl border border-border p-5 lg:p-6 mb-6">
            <h2 class="font-bold text-secondary mb-4">Carduri salvate</h2>

            <div class="space-y-4" id="cards-list">
                <!-- Loading -->
                <div class="animate-pulse flex items-center gap-4 p-4 border-2 border-border rounded-xl">
                    <div class="w-16 h-10 bg-muted/20 rounded-lg"></div>
                    <div class="flex-1">
                        <div class="h-4 bg-muted/20 rounded w-1/3 mb-2"></div>
                        <div class="h-3 bg-muted/20 rounded w-1/4"></div>
                    </div>
                </div>
            </div>
            ...
            <div id="no-cards" class="hidden text-center py-8">
                ...
                <p class="text-muted mb-2">Nu ai carduri salvate</p>
                <button onclick="openModal()" class="text-primary text-sm font-medium hover:underline">Adauga primul card</button>
            </div>
        </div>

        <!-- Other Payment Methods -->
        <div class="bg-white rounded-xl lg:rounded-2xl border border-border p-5 lg:p-6 mb-6">
            <h2 class="font-bold text-secondary mb-4">Alte metode de plata</h2>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                ...
                <div class="flex items-center justify-between p-4 bg-surface rounded-xl">
                    ...
                    <button class="text-sm text-primary font-medium hover:underline" id="apple-pay-btn">Conecteaza</button>
                </div>

                <!-- Google Pay -->
                <div class="flex items-center justify-between p-4 bg-surface rounded-xl">
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 bg-white border border-border rounded-xl flex items-center justify-center">
                    ...
                    <button class="text-sm text-primary font-medium hover:underline" id="google-pay-btn">Conecteaza</button>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl lg:rounded-2xl border border-border p-5 lg:p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <button onclick="editAddress()" class="text-sm text-primary font-medium hover:underline">Editeaza</button>
            </div>

            <div class="p-4 bg-surface rounded-xl" id="billing-address">
                <p class="text-muted text-sm">Nicio adresa salvata</p>
            </div>
        </div>

        <!-- Security Info -->
        <div class="mt-6 p-4 bg-success/5 border border-success/20 rounded-xl flex items-start gap-3">
    ...
    <div id="cardModal" class="modal fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="modal-content bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div class="p-5 lg:p-6 border-b border-border flex items-center justify-between">
                ...
                <button onclick="closeModal()" class="p-2 hover:bg-surface rounded-lg">
            ...
            <div class="p-5 lg:p-6">
                <!-- Card Preview -->
                <div class="credit-card p-5 mb-6 text-white">
                    <div class="flex justify-between items-start mb-8">
                        <div class="w-12 h-10 bg-gradient-to-br from-yellow-200 to-yellow-400 rounded"></div>
                        <svg class="w-12 h-8" viewBox="0 0 60 20" fill="white">
                            <text x="5" y="15" font-size="12" font-weight="bold" font-family="Arial">VISA</text>
                        </svg>
                    </div>
                    <p class="font-mono text-xl tracking-widest mb-4" id="cardNumberPreview">**** **** **** ****</p>
                    <div class="flex justify-between items-end">
                        <div>
                            <p class="text-xs text-white/60 uppercase">Titular</p>
                            <p class="font-medium" id="cardNamePreview">NUME PRENUME</p>
                        </div>
                        <div class="text-right">
                            <p class="text-xs text-white/60 uppercase">Expira</p>
                ...
                <form id="card-form" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-secondary mb-2">Numar card</label>
                        <input type="text" id="cardNumber" placeholder="1234 5678 9012 3456" maxlength="19" oninput="formatCardNumber(this)" class="w-full px-4 py-3 bg-surface border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-mono">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-secondary mb-2">Numele de pe card</label>
                        <input type="text" id="cardName" placeholder="ANDREI POPESCU" oninput="updateCardPreview()" class="w-full px-4 py-3 bg-surface border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary uppercase">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-secondary mb-2">Data expirarii</label>
                            <input type="text" id="cardExpiry" placeholder="MM/YY" maxlength="5" oninput="formatExpiry(this)" class="w-full px-4 py-3 bg-surface border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-mono">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-secondary mb-2">CVV</label>
                            <input type="text" id="cardCvv" placeholder="123" maxlength="4" class="w-full px-4 py-3 bg-surface border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-mono">
                        </div>
                    </div>
                    <div class="flex items-center gap-3 p-3 bg-surface rounded-xl">
                        <input type="checkbox" id="defaultCard" class="w-4 h-4 text-primary rounded border-border focus:ring-primary">
                        <label for="defaultCard" class="text-sm text-secondary">Seteaza ca metoda principala de plata</label>
                    </div>
                </form>
            </div>

            <div class="p-5 lg:p-6 border-t border-border flex gap-3">
                <button onclick="closeModal()" class="flex-1 py-3 bg-surface text-secondary font-semibold rounded-xl text-sm hover:bg-primary/10 hover:text-primary transition-colors">
                    Anuleaza
                </button>
                <button onclick="saveCard()" class="flex-1 btn btn-primary py-3 text-white font-semibold rounded-xl text-sm">
                    Salveaza cardul
                </button>
            </div>
        </div>
        ...
        if (user) {
            ...
            if (user.address || user.city) {
                document.getElementById('billing-address').innerHTML = `
                    ...
                    <p class="text-sm text-muted mt-1">${user.address || ''}</p>
        ...
        container.innerHTML = this.cards.map(card => `
            <div class="card-item ${card.is_default ? 'selected' : ''} border-2 ${card.is_default ? 'border-primary' : 'border-border'} rounded-xl p-4 cursor-pointer" onclick="PaymentsPage.selectCard(${card.id})">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button onclick="event.stopPropagation(); PaymentsPage.editCard(${card.id})" class="p-2 text-muted hover:text-secondary hover:bg-surface rounded-lg transition-colors" title="Editeaza">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </button>
                        <button onclick="event.stopPropagation(); PaymentsPage.deleteCard(${card.id})" class="p-2 text-muted hover:text-error hover:bg-error/10 rounded-lg transition-colors" title="Sterge">
