<?php
/**
 * GitHub Webhook Auto-Deploy (FTP-only servers) — teatru.tixello.ro
 *
 * Descarcă ZIP-ul branch-ului `teatru` și extrage fișierele în web root.
 ...
 *    - Payload URL: https://teatru.tixello.ro/_webhook-deploy.php
 *    - Content type: application/json
 *    - Secret: (același cu DEPLOY_SECRET)
 *    - Events: Just the push event
 * 4. Prima dată poți declanșa manual: https://teatru.tixello.ro/_webhook-deploy.php?test=1
 *
 * NOTĂ: necesită ca repo-ul GitHub să fie accesibil pentru download ZIP.
 *       Dacă repo-ul e privat, folosește în schimb deploy prin FTP direct
 *       (deploy-teatru.bat varianta FTP).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && !isset($_GET['test'])) {
    ...
    ?>
    <!DOCTYPE html><html><head><title>Deploy — teatru.tixello.ro</title>
    <style>body{font-family:system-ui,sans-serif;max-width:800px;margin:40px auto;padding:20px;background:#f5f5f5}.card{background:#fff;border-radius:8px;padding:20px;margin-bottom:20px;box-shadow:0 2px 4px rgba(0,0,0,.1)}pre{background:#1e1e1e;color:#d4d4d4;padding:15px;border-radius:6px;overflow:auto;font-size:13px;max-height:400px}.btn{display:inline-block;padding:10px 20px;background:#722F37;color:#fff;border-radius:6px;text-decoration:none;font-weight:500}</style>
    </head><body>
...
$zipFile = sys_get_temp_dir() . '/teatru_deploy_' . time() . '.zip';
...
$zip = new ZipArchive();
$extractPath = sys_get_temp_dir() . '/teatru_extract_' . time();
