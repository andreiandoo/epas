...
layout('account', ['title' => 'Bilete — ' . kit_cfg('site_name'), 'nav' => 'tickets'], function () { ?>
  <h1 class="kit-display" style="font-size:1.8rem;margin-bottom:1.25rem"><?= e(t('account.my_tickets')) ?></h1>
  <div x-data="{ items:[], loading:true, async load(){ const r=await KitProxy('acc-tickets'); this.items=(r&&r.data)||[]; this.loading=false; } }" x-init="load()">
    ...
    <div style="display:flex;flex-direction:column;gap:.75rem">
      <template x-for="t in items" :key="t.code">
        <div class="kit-ticket">
          ...
          <div class="kit-ticket__body">
            <span class="kit-badge" :style="t.is_subscription?'':'background:var(--kit-success);color:#fff'" x-text="t.is_subscription?<?= e(json_encode(t('account.subscription'))) ?>:<?= e(json_encode(t('account.valid'))) ?>"></span>
      ...
      <div x-show="!loading && items.length===0" class="kit-empty"><?= e(t('account.no_tickets')) ?></div>
    </div>
  </div>
<?php });
