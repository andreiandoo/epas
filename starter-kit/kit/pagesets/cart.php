...
function () { ?>
  <section class="kit-section"><div class="kit-container" x-data="{
    ...
    <h1 class="kit-display" style="font-size:clamp(1.8rem,4vw,2.6rem);margin-bottom:1.5rem">Coșul meu</h1>
    <?php component('step-indicator', ['steps' => ['Coș', 'Date', 'Plată'], 'current' => 1]); ?>
    <div style="display:grid;gap:2rem;grid-template-columns:1fr" class="kit-cart-layout">
      <div>
        ...
        <div x-show="items.length===0" class="kit-empty">Coșul este gol. <a href="<?= e(kit_cfg('cta_url','/')) ?>" class="kit-btn kit-btn--outline" style="margin-top:1rem">Vezi <?= e(kit_term('events','evenimente')) ?></a></div>
      </div>
      <div x-show="items.length>0" class="kit-ordersum">
        ...
        <a href="/finalizare" class="kit-btn kit-btn--primary" style="width:100%;margin-top:.75rem">Continuă</a>
      </div>
    </div>
  </div></section>
<?php });
