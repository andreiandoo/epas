...
/**
 * EPAS Metro Theme Toggle
 * Railway-inspired dark theme for admin panel
 */
(function() {
    const METRO_THEME_KEY = 'epas-metro-theme';

    // Check saved preference and apply theme on load
    function initMetroTheme() {
        const savedTheme = localStorage.getItem(METRO_THEME_KEY);
        if (savedTheme === 'enabled') {
            document.documentElement.classList.add('theme-metro');
        }
    }

    // Toggle Metro theme
    function toggleMetroTheme() {
        const html = document.documentElement;
        const isEnabled = html.classList.toggle('theme-metro');
        localStorage.setItem(METRO_THEME_KEY, isEnabled ? 'enabled' : 'disabled');
        return isEnabled;
    }

    // Enable Metro theme
    function enableMetroTheme() {
        document.documentElement.classList.add('theme-metro');
        localStorage.setItem(METRO_THEME_KEY, 'enabled');
    }

    // Disable Metro theme
    function disableMetroTheme() {
        document.documentElement.classList.remove('theme-metro');
        localStorage.setItem(METRO_THEME_KEY, 'disabled');
    }

    // Check if Metro theme is active
    function isMetroThemeActive() {
        return document.documentElement.classList.contains('theme-metro');
    }

    // Initialize on DOM ready
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initMetroTheme);
    } else {
        initMetroTheme();
    }

    // Also run immediately for faster visual response
    initMetroTheme();

    // Expose to global scope for console/UI access
    window.EPAS = window.EPAS || {};
    window.EPAS.toggleMetroTheme = toggleMetroTheme;
    window.EPAS.enableMetroTheme = enableMetroTheme;
    window.EPAS.disableMetroTheme = disableMetroTheme;
    window.EPAS.isMetroThemeActive = isMetroThemeActive;
})();
