...
const APP_VERSION = '2.2.0-dev.10';
...
// Sentry init runs BEFORE any component mounts — captures crashes from
// module-load onwards. DSN comes from expo config extra (set via app.json
// or env var). No DSN = silent no-op (dev builds without observability
// still work normally).
try {
  ...
  if (dsn) {
    initCrashReporter({
      dsn,
      release: `ambilet-scan-next@${APP_VERSION}`,
      environment: __DEV__ ? 'development' : 'production',
    });
  }
} catch (e) {
  // silent — crashReporter can't crash the boot
}
...
function MainTabs() {
  ...
  // Auto-logout — pauses during any modal that we don't want interrupted:
  // pending payment (money in flight), emergency, gate/staff mgmt, events
  // picker (operator actively choosing), and while shift is running WITH
  // active screen focus on Scanare (a live scan queue should never be
  // interrupted). autoLogoutMinutes = 0 disables the feature.
  const inactivityPausedBy = (
    !!showEmergency
    || !!showGateManager
    || !!showStaffAssignment
    || !!showStaff
  ...
  const { bumpActivity } = useInactivityTimer({
    ...
    enabled: !!user?.id && (autoLogoutMinutes || 0) > 0,
    pauseWhen: inactivityPausedBy,
    onExpire: () => {
      try { logout?.(); } catch {}
    },
  });
  ...
  return (
    <View
      ...
      // onStartShouldSetResponderCapture fires BEFORE child components see
      // the touch — perfect for a passive activity-tracker that doesn't
      // consume the event (we return false, so children still receive it).
      onStartShouldSetResponderCapture={() => { bumpActivity(); return false; }}
      onMoveShouldSetResponderCapture={() => { bumpActivity(); return false; }}
    >
      ...
      >
        {(!isStaff || hasPermission('orders')) && (
          <Tab.Screen name="Dashboard" options={{ tabBarLabel: 'Panou' }}>
            {(props) => (
              <ScreenErrorBoundary screenName="Panou">
        ...
        <Tab.Screen name="CheckIn" options={{ tabBarLabel: 'Scanare' }}>
          {(props) => (
            <ScreenErrorBoundary screenName="Scanare">
              <React.Suspense fallback={<LazyScreenFallback />}>
                <CheckInScreen {...props} />
              </React.Suspense>
            </ScreenErrorBoundary>
        ...
        <Tab.Screen name="Sales" options={{ tabBarLabel: 'Vânzare' }}>
          {(props) => (
            <ScreenErrorBoundary screenName="Vânzare">
              <React.Suspense fallback={<LazyScreenFallback />}>
                <SalesScreen {...props} />
              </React.Suspense>
            </ScreenErrorBoundary>
        ...
        {isAdmin && hasPermission('reports') && (
          <Tab.Screen name="Reports" options={{ tabBarLabel: 'Rapoarte' }}>
            {(props) => (
              <ScreenErrorBoundary screenName="Rapoarte">
                <React.Suspense fallback={<LazyScreenFallback />}>
                  <ReportsScreen {...props} />
                </React.Suspense>
              </ScreenErrorBoundary>
        ...
        <Tab.Screen name="Settings" options={{ tabBarLabel: 'Setări' }}>
          {(props) => (
            <ScreenErrorBoundary screenName="Setări">
              <React.Suspense fallback={<LazyScreenFallback />}>
                <SettingsScreen
                  {...props}
...
function withSuspense(LazyComp, screenName) {
  return (props) => (
    <ScreenErrorBoundary screenName={screenName || 'Ecran'}>
      <React.Suspense fallback={<LazyScreenFallback />}>
        <LazyComp {...props} />
      </React.Suspense>
    </ScreenErrorBoundary>
...
function VenueOwnerEventsStack() {
  return (
    <Stack.Navigator screenOptions={{ headerShown: false }}>
      <Stack.Screen name="VenueEventsList" component={withSuspense(VenueEventsScreen, 'Evenimente Locație')} />
      <Stack.Screen name="VenueEventDetail" component={withSuspense(VenueEventDetailScreen, 'Detaliu Eveniment')} />
      <Stack.Screen name="VenueTicketDetail" component={withSuspense(VenueTicketDetailScreen, 'Detaliu Bilet')} />
    </Stack.Navigator>
  );
}
...
// wrapRootApp = Sentry.wrap under the hood when the SDK is linked, plain
// component otherwise. Adds performance profiling + touch-based navigation
// breadcrumbs automatically.
export default wrapRootApp(App);
