...
export default function Header({ onNotificationPress, onOrganizerSwitched, pageTitle }) {
  const {
    ...
    pendingOfflineCount, flushOfflineQueue,
    pendingOfflineSalesCount = 0, flushOfflineSalesQueue,
  } = useApp();
  // Header badge shows the sum of both offline queues — scans + sales —
  // since the operator only cares "how many things are waiting to sync".
  const totalPending = (pendingOfflineCount || 0) + (pendingOfflineSalesCount || 0);
  const flushBoth = () => {
    try { flushOfflineQueue?.(); } catch {}
    try { flushOfflineSalesQueue?.(); } catch {}
  };
