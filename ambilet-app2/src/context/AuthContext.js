...
export function AuthProvider({ children }) {
  ...
  const applyLoginPayload = useCallback((payload) => {
    ...
    if (type === 'venue_owner') {
      ...
      // Sentry context — id only, no PII.
      setCrashUser(payload.venue_owner, payload.venue_owner?.id);
    } else {
      setUser(payload.organizer || null);
      setAvailableOrganizers(payload.available_organizers || []);
      setVenueOwner(null);
      setCrashUser(payload.organizer, payload.organizer?.marketplace_organizer_id || payload.organizer?.id);
    }
