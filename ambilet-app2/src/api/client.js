...
async function request(url, options = {}) {
  ...
  if (!response.ok) {
    ...
    // Report only 5xx (server bugs) to Sentry. 4xx are caller errors / user
    // input and rarely actionable; noise-only if reported. `require` lazy
    // to avoid a circular import (crashReporter → nothing → …).
    if (response.status >= 500) {
      try {
        const { captureError } = require('../services/crashReporter');
        captureError(error, { endpoint: url, status: response.status });
      } catch {}
    }
    throw error;
  }
...
// Multipart POST — for file uploads. Skips Content-Type header so fetch
// (React Native's stack) auto-generates the multipart boundary. Body must
// already be a FormData instance.
export async function apiMultipart(path, formData) {
  const rewritten = rewritePath(path);
  const url = `${BASE_URL}${rewritten}`;
