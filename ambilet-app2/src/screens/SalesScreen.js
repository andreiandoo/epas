...
export default function SalesScreen({ navigation }) {
  ...
  const processPayment = async (method) => {
    ...
    try {
      ...
      // OFFLINE PATH — no network: queue the sale for later sync. We
      // fake the success flow (no claim URL, no email capture — those
      // require the backend order id) but still record it in the local
      // history so the operator sees Sales Today totals include it.
      // Tap/Stripe requires network → refuses offline.
      if (!isOnline) {
        if (method === 'tap') {
          alert('Plata cu Stripe Tap necesită internet. Folosește Numerar sau Card POS și confirmă manual.');
          setIsProcessing(false);
          setPaymentMethod(null);
          return;
        }
        try {
          await enqueueOfflineSale({ eventId: selectedEvent?.id, payload: orderPayload });
        } catch (e) {
          alert('Nu am putut salva vânzarea offline. Reîncearcă.');
          setIsProcessing(false);
          setPaymentMethod(null);
          return;
        }
        // Record locally so the shift totals + Vânzări Azi list reflect it.
        try {
          const methodLabel = method === 'cash' ? 'Numerar' : 'Card';
          const desc = cartItems.map(it => `${it.quantity}x ${it.name}`).join(', ');
          addSale?.({
