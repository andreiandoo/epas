...
export default function CheckInScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    return () => nameSearchDebounceRef.current && clearTimeout(nameSearchDebounceRef.current);
  }, [nameQuery, showManualEntry, selectedEvent?.id]);
  ...
  // Attendee-name search — 300ms debounced against
  // /organizer/events/{id}/participants?search=X (already supports search).
  const [nameQuery, setNameQuery] = useState('');
  const [nameResults, setNameResults] = useState([]);
  const [nameSearching, setNameSearching] = useState(false);
  const nameSearchDebounceRef = useRef(null);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Hardware BT scanner support — off-screen invisible input that
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.manualEntryModal}>
              ...
              >
                <Text style={styles.checkInSubmitButtonText}>Check-in după cod</Text>
              </TouchableOpacity>

              {/* Path B — căutare după nume */}
              <View style={styles.nameSearchDivider}>
                <View style={styles.nameSearchDividerLine} />
                <Text style={styles.nameSearchDividerText}>SAU</Text>
                <View style={styles.nameSearchDividerLine} />
              </View>
              <Text style={styles.modalDescription}>
                ...
                <View style={styles.nameResultsBox}>
                  {nameSearching ? (
                    ...
                    nameResults.slice(0, 8).map((p, idx) => {
                      ...
                      const displayName = p.attendee?.name
                        || p.customer?.name
                        || p.name
                        || p.full_name
                        || 'Necunoscut';
                      ...
                      return (
                        <TouchableOpacity
                          key={p.id || `${code}-${idx}`}
                          style={[
                            styles.nameResultRow,
                            alreadyChecked && styles.nameResultRowDisabled,
                          ]}
                          onPress={() => {
                        ...
                        >
                          ...
                          {alreadyChecked ? (
                            <View style={styles.nameResultDoneBadge}>
                              <Text style={styles.nameResultDoneBadgeText}>Scanat</Text>
                            </View>
                          ) : (
                            <Text style={styles.nameResultCta}>Check-in →</Text>
                          )}
                        </TouchableOpacity>
                      );
                    })
...
const styles = StyleSheet.create({
  ...
  scannerFrame: {
    ...
    borderRadius: 24,
    borderWidth: 2,
    backgroundColor: 'rgba(20,10,10,0.02)',
    overflow: 'hidden',
    position: 'relative',
  },
  scannerPlaceholder: {
  ...
  nameSearchDividerLine: {
    flex: 1,
    height: 1,
    backgroundColor: colors.border,
  },
  nameSearchDividerText: {
    fontSize: 11,
    fontWeight: '700',
    color: colors.textTertiary,
    letterSpacing: 1,
  ...
  scanDetailsRow: {
    ...
    paddingVertical: 10,
    borderBottomWidth: 1,
    borderBottomColor: 'rgba(20,10,10,0.06)',
  },
  scanDetailsLabel: {
    fontSize: 14,
    color: colors.textSecondary,
