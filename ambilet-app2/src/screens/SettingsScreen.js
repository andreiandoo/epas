...
function AutoLogoutPicker() {
  const { autoLogoutMinutes, changeAutoLogoutMinutes } = useApp();
  const options = [
    { value: 0, label: 'Oprit' },
    { value: 5, label: '5 min' },
  ...
  return (
    <View style={{ gap: 8 }}>
      <Text style={styles.settingLabel}>Auto-logout după inactivitate</Text>
      <Text style={styles.settingDescription}>
        Dacă operatorul nu atinge ecranul un timp, contul se deconectează automat.
        Se pauzează în timpul plăților și modalelor active.
      </Text>
      <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
        {options.map(opt => {
          const active = autoLogoutMinutes === opt.value;
          return (
            <TouchableOpacity
              key={opt.value}
              activeOpacity={0.7}
              onPress={() => changeAutoLogoutMinutes(opt.value)}
              style={[
                styles.autoLogoutChip,
                active && styles.autoLogoutChipActive,
              ]}
            >
              <Text style={[
                styles.autoLogoutChipText,
                active && styles.autoLogoutChipTextActive,
              ]}>{opt.label}</Text>
            </TouchableOpacity>
          );
        })}
...
export default function SettingsScreen({ onShowGateManager, onShowStaffAssignment, onShowManual, appVersion }) {
  ...
  return (
    ...
    >
      ...
      {/* Securitate — auto-logout după inactivitate. Protejează contul dacă
          operatorul lasă telefonul pe masă între ture. */}
      ...
      {/* App Version */}
      <Text style={styles.versionText}>AmBilet Scan v{appVersion || '2.2.0-dev.3'}</Text>
