...
return new class extends Migration
{
        ...
        if (Schema::hasTable('orders')) {
            Schema::table('orders', function (Blueprint $table) {
                if (! Schema::hasColumn('orders', 'source_short_id')) {
                    /* FARA ->index() aici, deliberat. `orders` e un tabel mare de
                       productie, iar un CREATE INDEX obisnuit blocheaza scrierile
                       cat dureaza constructia — adica pica checkout-ul. Coloana
                       nullable in sine e instantanee pe Postgres.
                       Indexul se creeaza concurent, in migrarea
                       2026_08_07_120001_shorts_index_orders_source_short_concurrently. */
                    $table->unsignedBigInteger('source_short_id')->nullable();
                }
    ...
    public function down(): void
    {
        ...
        if (Schema::hasTable('orders')) {
            Schema::table('orders', function (Blueprint $table) {
                // indexul e scos de migrarea lui; aici doar coloanele
