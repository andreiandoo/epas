...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-50 flex flex-col w-64 transform -translate-x-full border-r bg-gradient-to-b from-slate-900 to-slate-800 sidebar lg:static border-muted lg:translate-x-0">
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto">
        <a href="/organizator/panou" class="sidebar-link <?= $currentPage === 'dashboard' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium <?= $currentPage !== 'dashboard' ? 'text-white' : '' ?>">
            <svg class="<?= $currentPage !== 'dashboard' ? 'text-muted' : 'text-white' ?> w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
            Dashboard
        </a>
        ...
        <a href="/organizator/vanzari" class="sidebar-link <?= $currentPage === 'sales' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium <?= $currentPage !== 'sales' ? 'text-white' : '' ?>">
            <svg class="<?= $currentPage !== 'sales' ? 'text-muted' : 'text-white' ?> w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
            Vanzari
        </a>
        <a href="/organizator/reports" class=" sidebar-link <?= $currentPage === 'reports' ? 'active' : '' ?> hidden items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium <?= $currentPage !== 'reports' ? 'text-white' : '' ?>">
            <svg class="<?= $currentPage !== 'reports' ? 'text-muted' : 'text-white' ?> w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
            Rapoarte
        </a>
        <a href="/organizator/documente" class="sidebar-link <?= $currentPage === 'documents' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium <?= $currentPage !== 'documents' ? 'text-white' : '' ?>">
            <svg class="<?= $currentPage !== 'documents' ? 'text-muted' : 'text-white' ?> w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            Documente
        </a>
        ...
        <div class="pt-4 mt-4 border-t border-muted">
            ...
            <a href="/organizator/promo" class="sidebar-link <?= $currentPage === 'promo' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium <?= $currentPage !== 'promo' ? 'text-white' : '' ?>">
                <svg class="<?= $currentPage !== 'promo' ? 'text-muted' : 'text-white' ?> w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                Coduri promotionale
            </a>
            <a href="/organizator/widget-uri" class="sidebar-link <?= $currentPage === 'widgets' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium <?= $currentPage !== 'widgets' ? 'text-white' : '' ?>">
                <svg class="<?= $currentPage !== 'widgets' ? 'text-muted' : 'text-white' ?> w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>
                Widget-uri Embed
            </a>
        </div>
        ...
        <div class="pt-4 mt-4 border-t border-muted">
            ...
            <a href="/organizator/setari" class="sidebar-link <?= $currentPage === 'settings' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium <?= $currentPage !== 'settings' ? 'text-white' : '' ?>">
                <svg class="<?= $currentPage !== 'settings' ? 'text-muted' : 'text-white' ?> w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                Cont & companie
            </a>
            <a href="/organizator/help" class="sidebar-link <?= $currentPage === 'help' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium <?= $currentPage !== 'help' ? 'text-white' : '' ?>">
                <svg class="<?= $currentPage !== 'help' ? 'text-muted' : 'text-white' ?> w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                Ajutor & suport
            </a>
        </div>
    </nav>
