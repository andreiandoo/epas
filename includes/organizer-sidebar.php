...
<!-- Ascunde linkurile condiÈ›ionate pÃ¢nÄƒ cÃ¢nd JS-ul detecteazÄƒ tipul organizatorului.
     Previne "flash of all sidebar links" la Ã®ncÄƒrcarea paginii. -->
<style>
...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-50 flex flex-col w-64 transform -translate-x-full border-r bg-gradient-to-b from-slate-900 to-slate-800 sidebar lg:static border-slate-700 lg:translate-x-0">
    ...
    <div class="px-5 py-3 border-b border-slate-700">
        <a href="/" class="flex items-center gap-2.5">
            ...
            <div>
                ...
                <span class="text-[10px] text-muted font-semibold block -mt-1">ORGANIZATOR</span>
            </div>
        </a>
    </div>
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto">
        <!-- LEISURE: Dashboard live PRIMUL Ã®n meniu -->
        <a data-org-show="leisure" href="/organizator/leisure-dashboard" style="display:none" class="sidebar-link <?= $currentPage === 'leisure_dashboard' ? 'active' : '' ?> items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group text-white">
            <svg class="w-5 h-5 text-muted group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
            Dashboard live
            <span class="w-2 h-2 ml-auto rounded-full bg-emerald-400 animate-pulse"></span>
        </a>
        ...
        <a data-org-show="leisure" href="/organizator/leisure-raport" style="display:none" class="sidebar-link <?= $currentPage === 'leisure_raport' ? 'active' : '' ?> items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group text-white">
            <svg class="w-5 h-5 text-muted group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            Raport
        </a>
        <a data-org-show="leisure" href="/organizator/leisure-rentals" style="display:none" class="sidebar-link <?= $currentPage === 'leisure_rentals' ? 'active' : '' ?> items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group text-white">
            <svg class="w-5 h-5 text-muted group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            ðŸ›¶ Curse bÄƒrci
        </a>
        ...
        <div data-org-show="standard" class="flex flex-col pt-4 mt-4 space-y-1 border-t border-slate-700">
            ...
            <a data-org-show="standard" href="/organizator/promo" class="sidebar-link <?= $currentPage === 'promo' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group <?= $currentPage !== 'promo' ? 'text-white' : '' ?>">
                <svg class="<?= $currentPage !== 'promo' ? 'text-muted' : 'text-white' ?> w-5 h-5 group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                Coduri promoÈ›ionale
            </a>
            <a data-org-show="standard" href="/organizator/widget-uri" class="sidebar-link <?= $currentPage === 'widgets' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group <?= $currentPage !== 'widgets' ? 'text-white' : '' ?>">
                <svg class="<?= $currentPage !== 'widgets' ? 'text-muted' : 'text-white' ?> w-5 h-5 group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>
                Widget-uri Embed
            </a>
        </div>
        ...
        <div class="flex flex-col pt-4 mt-4 space-y-1 border-t border-slate-700">
            ...
            <a href="/organizator/facturare" class="sidebar-link <?= $currentPage === 'billing' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group <?= $currentPage !== 'billing' ? 'text-white' : '' ?>">
                <svg class="<?= $currentPage !== 'billing' ? 'text-muted' : 'text-white' ?> w-5 h-5 group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                Facturare
            </a>
            <a href="/organizator/setari" class="sidebar-link <?= $currentPage === 'settings' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group <?= $currentPage !== 'settings' ? 'text-white' : '' ?>">
            ...
            <a data-org-show="standard" href="/organizator/help" class="sidebar-link <?= $currentPage === 'help' ? 'active' : '' ?> flex items-center gap-3 px-4 py-3 m-2 rounded-xl text-sm font-medium group <?= $currentPage !== 'help' ? 'text-white' : '' ?>">
    ...
    <div class="sticky bottom-0 p-3 border-t border-slate-700">
        <div class="flex items-center gap-3 p-2">
            ...
            <div class="flex-1 min-w-0">
                <p id="sidebar-org-name" class="text-sm font-semibold truncate text-slate-600">Organizator</p>
                <p id="sidebar-org-plan" class="text-xs truncate text-muted">Professional Plan</p>
            </div>
            <button onclick="BileteOnlineAuth.logoutOrganizer()" class="p-1.5 text-muted hover:text-error hover:bg-error/10 rounded-lg transition-colors" title="Deconectare">
...
window.addEventListener('load', async function() {
    // Wait for BileteOnlineAPI to be available (retry a few times)
    let retries = 0;
    while (typeof BileteOnlineAPI === 'undefined' && retries < 10) {
        await new Promise(resolve => setTimeout(resolve, 100));
        retries++;
    }

    if (typeof BileteOnlineAPI === 'undefined') {
        console.error('BileteOnlineAPI not available');
        return;
    }

    try {
        // Load events count
        const eventsResponse = await BileteOnlineAPI.get('/organizer/events');
        ...
        if (eventsResponse.success && events.length > 0) {
            ...
            // Show "LocaÈ›ie de agrement" link when at least one event has display_template === 'leisure_venue'
        ...
        let orgData = {};
        try {
            const meResp = await BileteOnlineAPI.get('/organizer/me');
            ...
            orgData = BileteOnlineAuth.getOrganizerData?.() || JSON.parse(localStorage.getItem('bileteonline_organizer_data') || '{}');
        }

        // Apply org type â€” toggle leisure vs standard sidebar links
