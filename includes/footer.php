...
<footer x-data="bileteFooter(<?= htmlspecialchars($footerAlpineSeed, ENT_QUOTES) ?>)"
    ...
    <section class="relative overflow-hidden bo-marquee border-y-2 border-paper/10 bg-vermilion text-paper" aria-label="Categorii populare">
    ...
    <section class="relative bo-grain">
        ...
        <div class="relative mx-auto max-w-[1500px] px-4 py-12 sm:px-6 sm:py-16 lg:py-20">
            ...
            <div class="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
                ...
                <section class="relative overflow-hidden rounded-[2rem] border-2 border-paper/15 bg-paper text-ink shadow-deep">
                    ...
                    <div class="relative grid gap-8 p-6 sm:p-8 lg:grid-cols-[1fr_260px] lg:p-10">
                        <div>
                            ...
                            <h2 class="mt-5 font-display text-5xl font-bold leading-[.85]">
                                Nu vinde doar bilete. Vinde motive bune să ieși din casă.
                            </h2>
                            <p class="max-w-3xl mt-5 text-lg leading-relaxed text-ink-soft">
                            ...
                            <div class="flex flex-wrap gap-3 mt-7">
                                <a href="/categorii" class="px-6 py-4 font-bold transition rounded-full bg-vermilion text-paper hover:bg-vermilion-d">Explorează categorii</a>
                                <a href="/orase" class="px-6 py-4 font-bold transition border-2 rounded-full border-ink text-ink hover:bg-ink hover:text-paper">Alege orașul</a>
                                <a href="/card-cadou" class="px-6 py-4 font-bold transition rounded-full bg-forest text-paper hover:bg-ink">Card cadou</a>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section class="rounded-[2rem] border-2 border-paper/15 bg-paper/10 p-6 shadow-deep sm:p-8">
                    <h2 class="text-3xl font-bold leading-none font-display">Primește idei de weekend înainte să întrebi „ce facem?”.</h2>
                    <p class="mt-3 text-paper/60">Ghiduri, activități noi, idei pentru copii, experiențe cadou și puncte bonus care expiră.</p>

                    <form class="mt-3 space-y-3" @submit.prevent="subscribe()">
                        <label class="sr-only" for="footer-email">Email</label>
                        <input id="footer-email" required type="email" x-model="email" placeholder="emailul tău" class="w-full px-5 py-4 font-bold transition border-2 rounded-full outline-none border-paper/15 bg-paper text-ink focus:border-vermilion">
                        <div class="grid gap-2 sm:grid-cols-[1fr_auto]">
                            <select x-model="city" class="px-5 py-4 font-bold border-2 rounded-full outline-none border-paper/15 bg-paper text-ink">
                            ...
                            <button type="submit" :disabled="sending" class="px-6 py-4 font-bold transition rounded-full bg-vermilion text-paper hover:bg-vermilion-d disabled:opacity-60">
                                <span x-text="sending ? 'Se trimite…' : 'Abonează-mă'"></span>
                            </button>
                        </div>
                        <p x-show="message" x-cloak x-transition class="px-4 py-3 text-sm font-bold rounded-2xl" :class="ok ? 'bg-mint text-forest' : 'bg-rose text-vermilion-d'" x-text="message"></p>
            ...
            <div class="mt-12 border-t border-paper/10 pt-7">
                <div class="grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end">
                    <div>
                        <a href="/" class="inline-flex items-center gap-2.5 group" aria-label="<?= htmlspecialchars(SITE_NAME, ENT_QUOTES) ?> — acasă">
                            <span class="grid h-10 w-10 place-items-center rounded-md bg-vermilion text-paper rotate-[-4deg] transition group-hover:rotate-[4deg]">
                                <svg viewBox="0 0 24 24" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M3 9a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2 2 2 0 0 0 0 4 2 2 0 0 1-2 2H5a2 2 0 0 1-2-2 2 2 0 0 0 0-4Z"/><path d="M9 7v10" stroke-dasharray="2 2"/></svg>
                            </span>
                            <span class="text-3xl font-bold font-display text-paper">bilete<span class="text-vermilion">.</span>online</span>
                        </a>
                        <p class="max-w-3xl mt-4 text-sm leading-relaxed text-paper/50">
                    ...
                    <div class="flex flex-wrap gap-2 lg:justify-end">
                        <a href="/termeni" class="px-4 py-2 text-sm font-bold transition border rounded-full border-paper/15 text-paper/60 hover:bg-paper hover:text-ink">Termeni</a>
                        <a href="/confidentialitate" class="px-4 py-2 text-sm font-bold transition border rounded-full border-paper/15 text-paper/60 hover:bg-paper hover:text-ink">Confidențialitate</a>
                        <a href="/cookies" class="px-4 py-2 text-sm font-bold transition border rounded-full border-paper/15 text-paper/60 hover:bg-paper hover:text-ink">Cookies</a>
                        <button type="button" @click="openCookiePreferences()" class="px-4 py-2 text-sm font-bold transition border rounded-full border-paper/15 text-paper/60 hover:bg-paper hover:text-ink">Setări cookies</button>
                        <a href="/contact" class="px-4 py-2 text-sm font-bold transition border rounded-full border-paper/15 text-paper/60 hover:bg-paper hover:text-ink">Contact</a>
                    </div>
                </div>

                <div class="flex flex-col gap-4 pt-5 text-xs border-t mt-7 border-paper/10 text-paper/30 sm:flex-row sm:items-center sm:justify-between">
