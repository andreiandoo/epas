...
<aside id="user-sidebar" class="mobile:z-[1000] fixed lg:static inset-y-0 right-0 z-50 w-72 lg:w-64 bg-white lg:bg-transparent transform translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out flex-shrink-0 overflow-y-auto">
    <div class="min-h-screen p-4 bg-white lg:rounded-2xl lg:border lg:border-border lg:sticky lg:top-24 lg:min-h-0 mobile:p-0">
        ...
        <div class="pt-4 mt-4 border-t border-border mobile:px-4">
            <button onclick="BileteOnlineAuth.logout();" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-error hover:bg-error/10 transition-colors w-full">
                <svg class="flex-shrink-0 w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Update user info from auth (cached in localStorage â€” possibly stale points)
    ...
    // last login â€” points earned later won't show until next login. Fetch fresh
    ...
    function refreshSidebarPoints() {
        if (typeof BileteOnlineAPI === 'undefined' || typeof BileteOnlineAuth === 'undefined') return;
        if (!BileteOnlineAuth.isLoggedIn || !BileteOnlineAuth.isLoggedIn()) return;
        if (!BileteOnlineAPI.customer || !BileteOnlineAPI.customer.getPoints) return;

        BileteOnlineAPI.customer.getPoints().then(response => {
            if (!response || !response.success || !response.data) return;
            const pointsData = response.data.points || response.data;
            const balance = pointsData.balance || 0;

            const sidebarEl = document.getElementById('sidebar-user-points');
            if (sidebarEl) sidebarEl.textContent = balance.toLocaleString();
            const headerEl = document.getElementById('header-user-points');
            if (headerEl) headerEl.textContent = balance.toLocaleString();

            // Persist so cached reads (e.g. BileteOnlineAuth.getUser().points) are correct on next nav.
            if (BileteOnlineAuth.updateCustomerData) {
                BileteOnlineAuth.updateCustomerData({ points: balance });
            }
        }).catch(() => {});
    }
